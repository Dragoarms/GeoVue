...
class GeoVue:
    ...
    def __init__(self):
        ...
        # Initialize depth validator once via helper (idempotent)
        if hasattr(self, "_initialize_depth_validator"):
            self.logger.info("Initializing depth validator via helper")
            self._initialize_depth_validator()
        else:
            self.logger.warning(
                "Depth validator initializer not found; running without depth validation"
            )
            self.depth_validator = None  # Ensure it exists even if not loaded
        ...
        if self.config_manager.is_first_run() or not self._check_configuration():
            ...
            # Initialize depth validator after FileManager is set up
            self._initialize_depth_validator()
    ...
    def _initialize_drillhole_data_manager(self):
        ...
        if compartment_folders or csv_files:
            try:
                ...
                import traceback
                self.logger.error(traceback.format_exc())
                # Set to None so other code can check
                self.data_coordinator = None
        else:
            self.logger.warning("No compartment folders or CSV files found - DataCoordinator not initialized")
            self.data_coordinator = None
        
        # BACKWARD COMPATIBILITY: Keep old drillhole_data_manager reference
        # pointing to geological_store for code that still uses it
        # TODO: Remove this once all code is migrated to DataCoordinator
        self.drillhole_data_manager = None  # Mark as deprecated
        
        self.logger.info("=" * 80)

    def _initialize_depth_validator(self):
        """Initialize depth validator if CSV can be found."""
        # Try to find drillhole_data.csv using FileManager helper
        csv_path = None
        
        # First try datasets folder (new location)
        if hasattr(self.file_manager, 'shared_paths') and self.file_manager.shared_paths:
            datasets_dir = self.file_manager.shared_paths.get('datasets')
            if datasets_dir:
                potential_path = os.path.join(datasets_dir, "drillhole_data.csv")
                if os.path.exists(potential_path):
                    csv_path = potential_path
                    self.logger.info(f"Found drillhole_data.csv in datasets folder: {csv_path}")
        
        # Fallback to register folder (old location) if not found
        if not csv_path:
            register_dir = self.file_manager.shared_paths.get('register') if hasattr(self.file_manager, 'shared_paths') else None
            if register_dir:
                potential_path = os.path.join(register_dir, "drillhole_data.csv")
                if os.path.exists(potential_path):
                    csv_path = potential_path
                    self.logger.warning(f"Found drillhole_data.csv in register folder (old location): {csv_path}")
                    self.logger.warning("Consider moving drillhole_data.csv to Drillhole Datasets folder")
        
        # Initialize validator if we found the CSV
        if csv_path and os.path.exists(csv_path):
            ...
            self.logger.warning("drillhole_data.csv not found in datasets, register, or config locations")
            self.depth_validator = None  # Ensure it exists even if no CSV
