...
class VisualizationDrawer:
                ...
                if rotation_angle != 0 or top_slope != 0 or bottom_slope != 0:
                    ...
                    if rotation_angle != 0:
                        ...
                        rotated_corners = np.dot(
                            rotation_matrix, rect_corners_homog.T
                        ).T
    ...
    def draw_markers(image: np.ndarray, **kwargs) -> np.ndarray:
        ...
        Args:
            ...
            **kwargs:
                ...
                - transformation_matrix: Optional 2x3 or 3x3 transformation matrix to apply to markers
        ...
        transformation_matrix = kwargs.get("transformation_matrix", None)

        # Transform markers if a transformation matrix is provided
        if transformation_matrix is not None:
            ...
            if isinstance(transformation_matrix, list):
                transformation_matrix = np.array(
                    transformation_matrix, dtype=np.float32
                )
            ...
            for marker_id, corners in markers.items():
                if isinstance(corners, np.ndarray):
                    ...
                    for corner in corners:
                        ...
                        if transformation_matrix.shape[0] == 2:
                            # 2x3 affine matrix
                    ...
                    transformed_markers[marker_id] = np.array(
                        transformed_corners, dtype=np.float32
                    )

            # Use transformed markers for drawing
            markers_to_draw = transformed_markers
        else:
            markers_to_draw = markers

        for marker_id, corners in markers_to_draw.items():
            if isinstance(corners, np.ndarray):
                # Draw marker outline
    ...
    def draw_scale_bar(image: np.ndarray, **kwargs) -> np.ndarray:
        ...
        text_size = cv2.getTextSize(f"{length_cm}cm", cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)[
            0
        ]
        cv2.putText(
            viz,
    ...
    @staticmethod
    def draw_composite(
        image: np.ndarray, operations: List[Dict[str, Any]]
    ) -> np.ndarray:
        """
