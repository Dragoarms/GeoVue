...
class RegisterStore:
    ...
    def _apply_operator(
        ...
        try:
            ...
            # ── Date operators ──────────────────────────────
            elif operator in ('on', 'not on', 'before', 'on or before', 'after', 'on or after'):
                data_date = self._parse_date(data_val)
                filter_date = self._parse_date(filter_val)
                if data_date is None or filter_date is None:
                    return False
                data_d = data_date.date() if hasattr(data_date, 'date') else data_date
                filter_d = filter_date.date() if hasattr(filter_date, 'date') else filter_date
                if operator == 'on':
                    return data_d == filter_d
    ...
    def _parse_date(value):
        """Parse a value into a datetime. Returns None on failure."""
        if value is None:
            return None
        if hasattr(value, 'date'):
            return value  # Already datetime/Timestamp
        try:
