...
class RCMetricsStore:
    ...
    def compute_from_dataframe(
        ...
        # Clear existing metrics only if starting fresh (not merging)
        if not merge:
        ...
        for idx, (_, row) in enumerate(df_lower.iterrows()):
            ...
            try:
                ...
                if merge and key in self._metrics_index:
                    logger.debug(
                        f"RC metrics key {key} overwritten by source '{source_name}'"
                    )
        ...
        if merge and self._metrics_index:
            # When merging, rebuild from the full _metrics_index to include all sources
            all_metrics_list = [m.to_dict() for m in self._metrics_index.values()]
            self._metrics_df = pd.DataFrame(all_metrics_list)
            ...
            logger.info(
                f"Rebuilt _metrics_df from full _metrics_index: "
                f"{len(all_metrics_list)} rows (added {len(metrics_list)} from '{source_name}')"
            )
        elif metrics_list:
