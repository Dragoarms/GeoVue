...
class IndexedDataSource:
    ...
    def load_from_dataframe(self, df: pd.DataFrame) -> bool:
        """
        Load from an existing DataFrame (e.g. from Snowflake) instead of CSV.

        Performs the same indexing as load() but skips file I/O.

        Args:
            df: DataFrame to index. Columns will be lowercased.
        ...
        try:
            ...
            if hole_col not in self._df.columns:
                logger.error(
                    f"Hole ID column '{hole_col}' not found in DataFrame. "
                    f"Available: {list(self._df.columns)[:10]}..."
                )
                return False

            # Create normalized hole column
            self._df["_hole_upper"] = (
                self._df[hole_col].astype(str).str.strip().str.upper()
            )
            ...
            index_desc = self._get_index_description()
            logger.info(
                f"  Loaded '{self.name}' from DataFrame: "
                f"{self._row_count:,} rows in {self._load_time:.2f}s. {index_desc}"
            )

            return True

        except Exception as e:
            logger.error(f"Error loading DataFrame for '{self.name}': {e}")
            import traceback
            logger.debug(traceback.format_exc())
            self._is_loaded = False
            return False
    ...
    def query(
        ...
        if not filters:
            ...
            if return_keys:
                ...
                if hole_col in df.columns and depth_col in df.columns:
                    subset = df[[hole_col, depth_col]].copy()
                    subset[hole_col] = subset[hole_col].astype(str).str.strip().str.upper()
                    subset[depth_col] = pd.to_numeric(subset[depth_col], errors='coerce')
                    subset = subset.dropna(subset=[depth_col])
                    all_keys = set(zip(subset[hole_col], subset[depth_col].astype(float)))
                return all_keys
            else:
                return df.copy()
        ...
        for flt in filters:
            ...
            try:
                if op in ('=', 'equals', '=='):
                    ...
                    data_type = flt.get('data_type', '')
                    if data_type == 'date':
                        date_col = pd.to_datetime(col_data, errors='coerce')
                        d1 = pd.to_datetime(val, errors='coerce')
                        d2 = pd.to_datetime(val2, errors='coerce')
                        if pd.isna(d1) or pd.isna(d2):
                            logger.warning(f"Could not parse date between values: {val}, {val2}")
                            continue
                        date_col_d = date_col.dt.normalize()
                        mask &= (date_col_d >= pd.Timestamp(d1).normalize()) & (date_col_d <= pd.Timestamp(d2).normalize())
                    else:
                ...
                # ── Date operators ──────────────────────────────
                elif op in ('on', 'not on', 'before', 'on or before', 'after', 'on or after'):
                    date_col = pd.to_datetime(col_data, errors='coerce')
                    filter_date = pd.to_datetime(val, errors='coerce')
                    if pd.isna(filter_date):
                        logger.warning(f"Could not parse date filter value: {val}")
                        continue
                    # Compare date-only (strip time component)
                    date_col_d = date_col.dt.normalize()
                    filter_d = pd.Timestamp(filter_date).normalize()
                    if op == 'on':
                        mask &= (date_col_d == filter_d)
                    elif op == 'not on':
                        mask &= (date_col_d != filter_d) | date_col.isna()
                    elif op == 'before':
                        mask &= (date_col_d < filter_d)
                    ...
                    date_col = pd.to_datetime(col_data, errors='coerce')
                    filter_d1 = pd.to_datetime(val, errors='coerce')
                    filter_d2 = pd.to_datetime(val2, errors='coerce')
                    if pd.isna(filter_d1) or pd.isna(filter_d2):
                        logger.warning(f"Could not parse date between values: {val}, {val2}")
                        continue
                    d1 = pd.Timestamp(filter_d1).normalize()
                    d2 = pd.Timestamp(filter_d2).normalize()
                    date_col_d = date_col.dt.normalize()
                    mask &= (date_col_d >= d1) & (date_col_d <= d2)
            except Exception as e:
        ...
        if return_keys:
            ...
            if hole_col in filtered_df.columns and depth_col in filtered_df.columns:
                subset = filtered_df[[hole_col, depth_col]].copy()
                subset[hole_col] = subset[hole_col].astype(str).str.strip().str.upper()
                subset[depth_col] = pd.to_numeric(subset[depth_col], errors='coerce')
                subset = subset.dropna(subset=[depth_col])
                result_keys = set(zip(subset[hole_col], subset[depth_col].astype(float)))
...
class GeologicalStore:
    ...
    def add_source_from_dataframe(
        ...
        """
        Add a data source from an in-memory DataFrame (e.g. from Snowflake).

        Uses the same schema inference as add_source() but skips file I/O.
        The DataFrame is loaded and indexed immediately.

        Args:
            df: DataFrame to load
            name: Source name for GeologicalStore registration
            schema: Optional predefined schema. If None, infers from DataFrame.

        Returns:
            True if loaded successfully, False otherwise
        """
        if df is None or df.empty:
            logger.warning(f"Empty DataFrame for source '{name}', skipping")
            return False

        if name in self._sources:
            logger.warning(f"Data source '{name}' already exists, replacing")

        # Infer schema from the DataFrame if not provided
        if schema is None:
            try:
                saved_schemas = self._load_saved_schemas()
                saved_data = saved_schemas.get(name)
                if saved_data:
                    try:
                        schema = DataSourceSchema.from_dict(saved_data)
                        schema.file_path = f"snowflake://{name}"
                        logger.info(f"Using saved schema for Snowflake source '{name}'")
                    except Exception:
                        schema = None

                if schema is None:
                    logger.info(f"Inferring schema for Snowflake source '{name}'...")
                    schema = infer_schema(df.head(1000), name, f"snowflake://{name}")
            except Exception as e:
                logger.error(f"Error inferring schema for '{name}': {e}")
                return False
        ...
        saved_schemas = self._load_saved_schemas()
        self._apply_saved_schema(name, schema, saved_schemas)

        # Create source and load from DataFrame
        source = IndexedDataSource(schema)
        success = source.load_from_dataframe(df)

        if success:
            self._sources[name] = source
            self._rebuild_column_cache()
            logger.info(f"Added Snowflake source '{name}': {len(df):,} rows")
        else:
            logger.error(f"Failed to index Snowflake source '{name}'")
            ...
            for source in sources_to_query:
                if source.is_loaded and source.df is not None:
                    ...
                    if hole_col in df.columns and depth_col in df.columns:
                        ...
                        subset[depth_col] = pd.to_numeric(subset[depth_col], errors='coerce')
                        subset = subset.dropna(subset=[depth_col])
                        all_keys.update(zip(subset[hole_col], subset[depth_col].astype(float)))
            return all_keys
