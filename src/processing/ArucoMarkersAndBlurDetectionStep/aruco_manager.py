...
class ArucoManager:
    ...
    def analyze_compartment_boundaries(
        ...
        viz_elements: Dict[str, Any] = {}
        self.logger.debug(f"DEBUG: viz_elements: {viz_elements})")
        ...
        if not corner_markers:
            ...
            # If we have compartment markers, use their average Y position
            if comp_markers and scale_px_per_cm and compartment_height_px:
                marker_tops = [
                    int(corners[:, 1].min()) for corners in comp_markers.values()
                ]
                average_top_y = int(np.mean(marker_tops))

                # bottom_y is 2cm *below* the marker top in canvas coords (canvas Y is reversed)
                bottom_y = average_top_y
                top_y = max(0, bottom_y - compartment_height_px)

                self.logger.info(
                    f"No corner markers - inferred vertical constraints from markers: "
                    f"top_y={top_y}, bottom_y={bottom_y} based on average top marker Y={average_top_y} "
                    f"and compartment height={compartment_height_px}px"
                )
            elif compartment_height_px:
                # No marker data, fallback to centering
                center_y = image.shape[0] // 2
                top_y = max(0, center_y - compartment_height_px // 2)
                bottom_y = min(image.shape[0], center_y + compartment_height_px // 2)
                self.logger.info(
                    f"No corner or compartment markers - using centered height of {compartment_height_px}px"
                )
            else:
                # Worst case fallback
            ...
            top_y_from_top_markers = []
            bottom_y_from_bottom_markers = []

            for mid, corners in corner_markers.items():
                marker_top_y = int(np.min(corners[:, 1]))
                marker_bottom_y = int(np.max(corners[:, 1]))

                if mid in top_corner_ids:
                    # Top marker → extrapolate DOWN
                    interpolated_bottom_y = (
                        marker_top_y + compartment_height_px
                        if compartment_height_px
                        else marker_bottom_y
                    )
                    top_y_from_top_markers.append(marker_top_y)
                    bottom_y_from_bottom_markers.append(interpolated_bottom_y)
                    self.logger.debug(
                        f"Top marker {mid} at y={marker_top_y}, inferring bottom_y={interpolated_bottom_y}"
                    )
                elif mid in bottom_corner_ids:
                    # Bottom marker → extrapolate UP
                    interpolated_top_y = (
                        marker_bottom_y - compartment_height_px
                        if compartment_height_px
                        else marker_top_y
                    )
                    bottom_y_from_bottom_markers.append(marker_bottom_y)
                    top_y_from_top_markers.append(interpolated_top_y)
                    self.logger.debug(
                        f"Bottom marker {mid} at y={marker_bottom_y}, inferring top_y={interpolated_top_y}"
                    )

            # If both types are present: average them
            if top_y_from_top_markers and bottom_y_from_bottom_markers:
                mean_top_y = int(np.mean(top_y_from_top_markers))
                mean_bottom_y = int(np.mean(bottom_y_from_bottom_markers))
                current_height = mean_bottom_y - mean_top_y

                # If we have scale, enforce correct height
                if (
                    compartment_height_px
                    and abs(current_height - compartment_height_px) > 5
                ):
                    center_y = (mean_top_y + mean_bottom_y) // 2
                    top_y = max(0, center_y - compartment_height_px // 2)
                    bottom_y = min(
                        image.shape[0], center_y + compartment_height_px // 2
                    )
                    self.logger.info(
                        f"Inferred both top/bottom markers: Adjusted vertical range to {top_y}-{bottom_y} (centered)"
                    )
                else:
                    top_y = mean_top_y
                    bottom_y = mean_bottom_y
                    self.logger.info(
                        f"Inferred both top/bottom markers: Using raw average range {top_y}-{bottom_y}"
                    )
            else:
                # Fallback to single direction
                top_y = (
                    int(np.min(top_y_from_top_markers)) if top_y_from_top_markers else 0
                )
                bottom_y = (
                    int(np.max(bottom_y_from_bottom_markers))
                    if bottom_y_from_bottom_markers
                    else image.shape[0]
                )

                self.logger.info(
                    f"Inferred single direction vertical range: top_y={top_y}, bottom_y={bottom_y}"
                )
        ...
        for marker in marker_data:
            ...
            if not compartment_width_px or compartment_width_px <= 0:
                self.logger.warning(
                    "Missing scale-based width. Falling back to marker-based width."
                )
                compartment_width = int(marker["width"])
            else:
                compartment_width = compartment_width_px

            half_width = compartment_width // 2

            left_boundary = max(0, marker["center_x"] - half_width)
            right_boundary = min(image.shape[1], marker["center_x"] + half_width)

            boundary = (left_boundary, top_y, right_boundary, bottom_y)
            compartment_boundaries.append(boundary)

            self.logger.debug(
                f"Compartment {marker['id']}: center_x={marker['center_x']} → bounds=({left_boundary}, {top_y}, {right_boundary}, {bottom_y}) width={compartment_width}"
            )
        ...
        if metadata and "boundary_adjustments" in metadata:
            ...
            # Update top and bottom boundaries TODO - check these think it's wrong...
        ...
        if smart_cropping and compartment_boundaries:
            ...
            fixed_boundaries = metadata.get("fixed_boundaries", {}) if metadata else {}
        ...
        result = {
            ...
            "wall_detection_performed": smart_cropping,
            "wall_detection_results": viz_elements,
            # Processing metadata
            "compartment_interval": compartment_interval,
        }
    ...
    def _refine_balanced_boundaries(
        ...
        fixed_boundaries: Dict[int, Tuple[int, int, int, int]] = None,
        ...
        # If we have fixed boundaries, check if this is just a boundary removal
        if fixed_boundaries and boundary_to_marker:
            # Check if we're missing any boundaries that should be there
            expected_markers = set(range(4, 24))  # All compartment markers
            current_markers = set(boundary_to_marker.values())
            missing_markers = expected_markers - current_markers

            if missing_markers:
                # We have removed boundaries - don't redistribute, just return as-is
                self.logger.debug(
                    f"Boundaries removed for markers {missing_markers}, keeping other boundaries fixed"
                )
                return boundaries
        ...
        if scale_px_per_cm:
            ...
            self.logger.debug(f"Using scale-based margin: {margin}px (8mm)")
        ...
        if scale_px_per_cm:
            # Allow adjustment up to 5mm
            max_adjustment_px = int(0.5 * scale_px_per_cm)
        else:
            ...
            for idx in indices:
                ...
                # Case 4: No walls detected
                    ...
                    if scale_px_per_cm and width < int(
                        2 * scale_px_per_cm * 0.95
                    ):  # Less than 95% of 2cm
                        target_width = int(2 * scale_px_per_cm)
