...
# Don't force Agg backend - let it use the default TkAgg for Tkinter apps
...
class DrillholeDataVisualizer:
    ...
    def generate_compartment_visualization(
        self,
    ...
    def _generate_stitchable_plot(
        self, config: PlotConfig, data: Dict[str, Any], height: int
    ) -> Optional[np.ndarray]:
        ...
        if config.columns and config.columns[0] in data:
            ...
            if config.width > 40:
                ...
                if text_size[0] < config.width - 10:
                    ...
                    cv2.putText(
                        column, text, (text_x, text_y), font, 0.4, text_color, 1
                    )
        ...
        for col, value in values.items():
            ...
            if bar_width > 20:
                ...
                if text_size[0] < bar_width - 4:
                    ...
                    cv2.putText(
                        column, text, (text_x, text_y), font, 0.3, text_color, 1
                    )
        ...
        for i, col in enumerate(config.columns):
            if col in full_data.columns:
                ...
                if len(plot_data) > 0:
                    # Plot line
                    ax.plot(
                        plot_data[col], plot_data["depth"], label=col, linewidth=1.5
                    )
    ...
    def _create_scatter_plot(
        ...
        if x_col in full_data.columns and y_col in full_data.columns:
            ...
            if len(plot_data) > 0:
                ...
                # Plot all points
                ax.scatter(
                    plot_data[~current_mask][x_col],
                    plot_data[~current_mask][y_col],
                    c="gray",
                    alpha=0.5,
                    s=20,
                    label="Other intervals",
                )
    ...
    def create_color_legend(
        self, width: int = 200, max_height: int = 400
    ) -> np.ndarray:
        """
        ...
        for config in self.plot_configs:
            if config.color_map and config.show_legend:
                ...
                ax.text(
                    0.1,
                    y_pos,
                    config.title or config.plot_type.value,
                    fontsize=10,
                    fontweight="bold",
                )
                y_pos -= 0.05
        ...
        if img_bgr.shape[1] != width:
            ...
            img_bgr = cv2.resize(
                img_bgr, (width, new_height), interpolation=cv2.INTER_AREA
            )
