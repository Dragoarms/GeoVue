...
def _debug_log(msg: str, data: dict, hypothesis_id: str = ""):
    try:
        with open(r"c:\Users\georg\OneDrive\Home Python\GeoVue 26_01_27\.cursor\debug.log", "a", encoding="utf-8") as f:
            f.write(json.dumps({"sessionId": "debug-session", "hypothesisId": hypothesis_id, "location": "collar_map", "message": msg, "data": data, "timestamp": int(time.time() * 1000)}) + "\n")
    except Exception:
        pass
# #endregion
...
def _build_map_points(
    ...
    # #region agent log
    _debug_log("_build_map_points entry", {"collar_rows": len(collar_df), "hole_col": hole_col, "n_logger_holes": len(logger_holes), "n_all_holes": len(all_holes)}, "E")
    # #endregion
    if collar_df.empty:
    ...
    # #region agent log
    _debug_log("columns resolved", {"east_col": east_col, "north_col": north_col}, "D")
    # #endregion
    ...
    # #region agent log
    _debug_log("heuristic", {"min_x": min_x, "max_x": max_x, "min_y": min_y, "max_y": max_y, "outside_wgs84": outside_wgs84, "looks_like_utm": looks_like_utm, "looks_projected": looks_projected, "PYPROJ_AVAILABLE": PYPROJ_AVAILABLE, "n_rows": len(df)}, "A")
    # #endregion
    ...
    if looks_projected and PYPROJ_AVAILABLE:
        ...
        def try_reproject(zone: int, north: bool, use_swapped_xy: bool = False):
            ...
            # #region agent log
            _debug_log("reproject success", {"zone": zone, "north": north, "use_swapped_xy": use_swapped_xy, "n_points": len(points)}, "C")
            # #endregion
        ...
        # Try UTM zone 33 North (WGS84 Z33N, common) first
        primary_zones = [utm_zone, DEFAULT_UTM_ZONE, 32, 31, 34] if utm_zone is None else [utm_zone, DEFAULT_UTM_ZONE, 32, 31, 34]
        primary_zones = [z for z in primary_zones if z is not None]
        primary_zones = list(dict.fromkeys(primary_zones))
        last_error = None
        # 1) Try primary zones with configured hemisphere, then inferred (normal column order)
        for zone in primary_zones:
            for north in (utm_north, inferred_north):
                try:
                    return try_reproject(zone, north, use_swapped_xy=False)
                except Exception as e:
                    last_error = e
        ...
        for north in (inferred_north, not inferred_north):
            for zone in range(1, 61):
                try:
                    ...
                    last_error = e
        # 3) Retry with swapped east/north (some datasets have columns reversed)
        for zone in primary_zones:
            for north in (utm_north, inferred_north):
                try:
                    return try_reproject(zone, north, use_swapped_xy=True)
                except Exception as e:
                    last_error = e
        for north in (inferred_north, not inferred_north):
            for zone in range(1, 61):
        ...
        if last_error is not None:
            # #region agent log
            _debug_log("reproject failed all zones", {"last_error": str(last_error), "last_error_type": type(last_error).__name__}, "C")
            # #endregion
            logger.warning(
                "UTM reprojection failed for all zones and column order (%s), map may show projected coords warning",
                last_error,
            )

    # No reprojection: pass through as-is (or already WGS84)
    # #region agent log
    _debug_log("pass-through path", {"looks_projected": looks_projected, "n_points": len(df)}, "A")
    # #endregion
    ...
    # #region agent log
    _debug_log("_build_map_points return", {"has_coords": True, "warn_projected": warn_projected, "n_points": len(points)}, "A")
    # #endregion
...
def _render_map(map_data: Dict[str, Any]) -> str:
    ...
    """
    # #region agent log
    _debug_log("_render_map", {"has_coords": map_data.get("has_coords"), "warn_projected": map_data.get("warn_projected"), "n_points": len(map_data.get("points", []))}, "A")
    # #endregion
