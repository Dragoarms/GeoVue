...
from .outliers import compute_hybrid_outlier_scores, predict_most_likely_strat

logger = logging.getLogger(__name__)

LOGGING_INTERVAL_SOURCE_PRIORITY = [
...
def fill_empty_logger_values(df: pd.DataFrame, logger_col: str, hole_col: str) -> pd.DataFrame:
    ...
    if hole_col in result.columns and empty_mask.any():
        ...
        sort_cols = [hole_col]
        depth_from_col = ColumnResolver(result).get("depth_from")
        if depth_from_col and depth_from_col in result.columns:
            sort_cols.append(depth_from_col)
        ordered = result.sort_values(sort_cols, kind="mergesort")
        ordered_empty = ordered[logger_col].isna() | (ordered[logger_col].astype(str).str.strip() == '')
        ordered_values = ordered[logger_col].mask(ordered_empty, pd.NA)
        filled = ordered_values.groupby(ordered[hole_col], sort=False).transform(lambda x: x.ffill().bfill())
        result.loc[filled.index, logger_col] = filled
        empty_mask = result[logger_col].isna() | (result[logger_col].astype(str).str.strip() == '')
...
def build_merged_qaqc_dataframe(
    ...
    primary_source = _find_assay_source_name(data_coordinator.geological_store)
    if not primary_source:
        available = data_coordinator.geological_store.list_sources()
        raise ValueError(
            "No assay source found for logging review report. "
...
def _get_source_case_insensitive(geo_store, source_name: str):
    available = geo_store.list_sources() if geo_store else []
    for actual_name in available:
        if actual_name.lower() == source_name.lower():
            return actual_name, geo_store.get_source(actual_name)
    return None, None


def _resolve_interval_key_columns(df: pd.DataFrame) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    resolver = ColumnResolver(df)
    return resolver.get("hole_id"), resolver.get("depth_from"), resolver.get("depth_to")


def _has_interval_keys(df: pd.DataFrame) -> bool:
    hole_col, from_col, to_col = _resolve_interval_key_columns(df)
    return bool(hole_col and from_col and to_col)


def _has_chemistry_columns(df: pd.DataFrame) -> bool:
    return bool(resolve_chemistry_columns(df))


def _looks_like_logging_interval_source(df: pd.DataFrame) -> bool:
    if not _has_interval_keys(df):
        return False
    resolver = ColumnResolver(df)
    logging_columns = [
        resolve_logger_column(df),
        resolver.get("strat"),
        resolver.get("stratsum"),
        resolver.get("lithology"),
        resolver.get("loggeddate"),
    ]
    return any(logging_columns)


def _find_logging_interval_source(geo_store) -> Optional[Tuple[str, pd.DataFrame]]:
    if not geo_store:
        return None
    for preferred_name in LOGGING_INTERVAL_SOURCE_PRIORITY:
        source_name, src = _get_source_case_insensitive(geo_store, preferred_name)
        if (
            source_name
            and src
            and src.df is not None
            and not src.df.empty
            and _has_interval_keys(src.df)
        ):
            logger.info("Logging interval source selected: %s", source_name)
    ...
    for source_name in geo_store.list_sources():
        ...
        if (
            ...
            and src.df is not None
            and not src.df.empty
            and _looks_like_logging_interval_source(src.df)
        ):
            logger.info("Logging interval source auto-detected: %s", source_name)
            return source_name, src.df.copy()
    return None


def _find_exgeology_rc_source(geo_store) -> Optional[Tuple[str, pd.DataFrame]]:
    """Backward-compatible wrapper for old callers/tests."""
    return _find_logging_interval_source(geo_store)


def _find_assay_source_name(geo_store) -> Optional[str]:
    if not geo_store:
        return None
    for preferred_name in ASSAY_SOURCE_PRIORITY:
        source_name, src = _get_source_case_insensitive(geo_store, preferred_name)
        if (
            source_name
            and src
            ...
            and not src.df.empty
            and _has_interval_keys(src.df)
            and _has_chemistry_columns(src.df)
        ):
            logger.info("Assay source selected: %s", source_name)
            return source_name
    ...
    for source_name in geo_store.list_sources():
        ...
        if (
            ...
            logger.info("Assay source auto-detected: %s", source_name)
            return source_name
    return None
...
def _build_logging_interval_dataframe(
    ...
    exgeo = _find_logging_interval_source(geo_store)
    if not exgeo:
        available = geo_store.list_sources() if geo_store else []
        raise ValueError(
            "Logging interval source not found for report dataframe. "
            f"Available sources: {available}"
        )
    ...
    if not hole_col or not depth_from_col or not depth_to_col:
        raise ValueError(
            f"Logging interval source '{source_name}' missing required hole/from/to columns."
        )
...
def prepare_logging_review_data(
    ...
    merged_df["most_likely_strat"] = predict_most_likely_strat(
        merged_df,
        strat_col=strat_col,
        chem_cols=chem_actual_cols,
        min_group_size=10,
