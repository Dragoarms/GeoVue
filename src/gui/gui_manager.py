...
class GUIManager:
    ...
    def _load_theme_preference(self):
        ...
        try:
            ...
            config_path = os.path.join(
                self.file_manager.base_dir, "Scripts", "config.json"
            )
    ...
    def configure_ttk_styles(self, root=None):
        ...
        style.configure(
            "TButton", background=colors["secondary_bg"], foreground=colors["text"]
        )
        style.map(
            "TButton",
        ...
        style.configure(
            "TCheckbutton", background=colors["background"], foreground=colors["text"]
        )
        style.map(
            "TCheckbutton",
    ...
    def create_modern_button(
        ...
        button = ModernButton(
            ...
            fonts=self.fonts,  # <-- Pass the fonts dictionary
        )
    ...
    def create_combobox(
        self, parent, variable, values, width=10, readonly=True, validate_func=None
    ):
                ...
                for name, widget in self.themed_widgets.items():
                    try:
                        ...
                        if not hasattr(widget, "winfo_exists") or widget.winfo_exists():
                            if hasattr(widget, "update_theme") and callable(
                                getattr(widget, "update_theme")
                            ):
                ...
                try:
                    ...
                    if isinstance(widget, CollapsibleFrame):
                        if hasattr(widget, "update_theme"):
                            self.logger.debug(
                                "   🔁 Using CollapsibleFrame.update_theme()"
                            )
                            widget.update_theme(**self.theme_colors)
                        else:
                            self.logger.debug(
                                "   🧪 Falling back to _update_collapsible_frame()"
                            )
                            self._update_collapsible_frame(
                                widget, path, self.theme_colors
                            )
                        ...
                        and widget.__class__.__name__ == "ModernNotebook"
                    ):
                        ...
                        widget.config(
                            **{k: v for k, v in theme_kwargs.items() if v is not None}
                        )
                    ...
                    self.logger.error(
                        f"❌ Error updating widget at {path} ({widget_type}): {e}"
                    )
        ...
        if not style.layout(custom_style):
            ...
            style.map(
                ...
                selectbackground=[
                    (
                        "readonly",
                        theme_colors.get("selection_bg", theme_colors["accent_blue"]),
                    )
                ],
                selectforeground=[
                    ("readonly", theme_colors.get("selection_fg", "#ffffff"))
                ],
    ...
    def _update_collapsible_frame(self, frame, name, theme_colors):
        ...
        try:
            ...
            # Update header frame
            if hasattr(frame, "header_frame") and isinstance(
                frame.header_frame, tk.Frame
            ):
            ...
            if hasattr(frame, "toggle_button") and isinstance(
                frame.toggle_button, tk.Label
            ):
                frame.toggle_button.config(
                    bg=theme_colors["secondary_bg"], fg=theme_colors["accent_blue"]
                )

            # Update header label
            if hasattr(frame, "header_label") and isinstance(
                frame.header_label, tk.Label
            ):
                frame.header_label.config(
                    bg=theme_colors["secondary_bg"], fg=theme_colors["text"]
                )

            # Update content frame
            if hasattr(frame, "content_frame") and isinstance(
                frame.content_frame, tk.Frame
            ):
                frame.content_frame.config(bg=theme_colors["background"])
    ...
    def setup_menubar(self, root, menus):
        ...
        menubar = tk.Menu(root, bg=colors["menu_bg"], fg=colors["menu_fg"])
        ...
        for menu_name, menu_items in menus.items():
            ...
            for item in menu_items:
                if item["type"] == "command":
                    menu.add_command(
                        label=self.t(item["label"]), command=item["command"]
                    )
                    ...
                    menu.add_cascade(label=self.t(item["label"]), menu=submenu)
    ...
    def create_main_window(self, root, title, width=1200, height=900, center=True):
        ...
        title_label = ttk.Label(header_frame, text=self.t(title), style="Title.TLabel")
        ...
        scrollbar = ttk.Scrollbar(
            content_outer_frame, orient="vertical", command=canvas.yview
        )
    ...
    def create_button_row(
        self, parent, button_configs, side="bottom", anchor="se", padx=10, pady=10
    ):
        """
