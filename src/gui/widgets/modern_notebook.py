...
class ModernNotebook:
    ...
    def add(self, page_widget: tk.Frame, text: str, **kwargs):
        ...
        # Reparent page to the page container
        page_widget.pack_forget()
        page_widget.grid_forget() 
        page_widget.place_forget()
        
        # Store original parent for compatibility
        page_widget._original_parent = page_widget.master
        
        # Reparent to page_container using place temporarily
        page_widget.place(in_=self.page_container, x=0, y=0)
        page_widget.place_forget()
    ...
    def _switch_to_tab(self, index: int):
        ...
        if index in self.pages:
            # Make sure page is parented to page_container
            self.pages[index].pack(in_=self.page_container, fill=tk.BOTH, expand=True, padx=0, pady=0)
    ...
    def update_theme(self, theme_colors: dict):
        ...
        self.page_container.config(bg=theme_colors["background"])
