...
class MinimapCanvas(tk.Canvas):
    ...
    def __init__(self, parent, config_manager, gui_manager, width=500, height=400, **kwargs):
        ...
        self.assay_collar_color = self.config_manager.get('correlation_assay_collar_color', '#2ECC71')
        self.no_assay_collar_color = self.config_manager.get('correlation_no_assay_collar_color', '#F1C40F')
        ...
        self.configured_min_zoom = float(self.config_manager.get('correlation_zoom_min', 0.001))
        self.min_zoom = self.configured_min_zoom
    ...
    def _debug_enabled(self) -> bool:
        """Return whether verbose minimap diagnostics should be printed/logged."""
        try:
            return bool(self.config_manager.get('correlation_debug_minimap', False))
        except Exception:
            return False

    def _debug_log(self, message: str) -> None:
        """Emit verbose minimap diagnostics only when explicitly enabled."""
    ...
    def zoom_to_fit(self):
        ...
        if fit_zoom > 0:
            self.min_zoom = min(self.configured_min_zoom, fit_zoom)
    ...
    def render(self):
        ...
        if self._debug_enabled():
            ...
            if len(stack) >= 2:
                ...
                caller_info = "unknown"
            self._debug_log(
                f"[RENDER] START zoom={self.zoom_level:.6f} called from {caller_info}"
            )
        ...
        for idx, row in self.collar_data.iterrows():
            ...
            has_assays = self._assay_status_from_row(row)
            ...
            self._draw_collar(row['x'], row['y'], hole_id, is_selected, has_assays)
            collars_drawn += 1
        
        self._debug_log(
            f"[RENDER] Drew {collars_drawn} collars, {collars_onscreen} visible "
            f"on screen (canvas {canvas_w}x{canvas_h}); "
            f"items={len(self.collar_items)}, labels={len(self.collar_labels)}"
        )
    ...
    def _draw_collar(
        self,
        map_x: Union[int, float],
        map_y: Union[int, float],
        hole_id: str,
        is_selected: bool,
        has_assays: Optional[bool] = None,
    ):
        ...
        Args:
            ...
            has_assays: True if assay rows are available, False if not, None if unknown
        """
        canvas_x, canvas_y = self.map_to_canvas(map_x, map_y)
        
        size = self.selected_collar_size if is_selected else self.collar_size
        color = self._get_collar_fill_color(is_selected, has_assays)
    ...
    def _coerce_optional_bool(value: Any) -> Optional[bool]:
        """Coerce common dataframe bool representations."""
        if value is None:
            return None
        try:
            if pd.isna(value):
    ...
    def _assay_status_from_row(self, row: pd.Series) -> Optional[bool]:
        """Read assay status from a collar row if available."""
        if "has_assays" not in row.index:
            return None
        return self._coerce_optional_bool(row.get("has_assays"))

    def _get_collar_fill_color(self, is_selected: bool, has_assays: Optional[bool]) -> str:
        """Choose collar fill color from selection and assay status."""
        if is_selected:
            return self.selected_collar_color
        if has_assays is True:
            return self.assay_collar_color
        if has_assays is False:
            return self.no_assay_collar_color
    ...
    def _draw_status_info(self):
        ...
        if self.collar_data is not None and "has_assays" in self.collar_data.columns:
            legend_y = 30
            marker_size = 8
            assay_marker = self.create_oval(
                10,
                legend_y,
                10 + marker_size,
                legend_y + marker_size,
                fill=self.assay_collar_color,
                outline="black",
                width=1,
            )
            assay_text = self.create_text(
                24,
                legend_y - 2,
                text="Assays back",
                font=('Arial', 8),
            ...
            no_assay_marker = self.create_oval(
                10,
                legend_y + 16,
                10 + marker_size,
                legend_y + 16 + marker_size,
                fill=self.no_assay_collar_color,
                outline="black",
                width=1,
            )
            no_assay_text = self.create_text(
                24,
                legend_y + 14,
                text="No assays",
                font=('Arial', 8),
    ...
    def _on_draw_end(self, event):
        ...
        self._debug_log(f"[DRAW_END] Shift held: {append_selection}, event.state: {event.state}")
    ...
    def update_selection_from_line(self, append=False):
        ...
        self._debug_log(
            f"[UPDATE_SELECTION] append={append}, existing={len(self.selected_collar_ids)}, "
            f"new={len(new_hole_ids)}"
        )
        ...
        if append:
            ...
            self._debug_log(
                f"[UPDATE_SELECTION] Toggle mode: added {added}, removed {removed}, "
                f"total={len(self.selected_collar_ids)}"
            )
        else:
            # Replace selection
            self.selected_collar_ids = new_hole_ids
            self._debug_log(f"[UPDATE_SELECTION] Replaced with {len(self.selected_collar_ids)} holes")
    ...
    def _on_mousewheel(self, event):
        ...
        self._debug_log(
            f"[MOUSEWHEEL] Zoom {direction}: {old_zoom:.6f} -> {new_zoom:.6f} "
            f"(factor={zoom_factor})"
        )
        ...
        self._debug_log(f"[MOUSEWHEEL] New pan: ({self.pan_offset_x:.1f}, {self.pan_offset_y:.1f})")
