...
class LithologyGridCanvas:
    ...
    def _on_mousewheel(self, event):
        ...
        # Only scroll if not holding Ctrl
        if not (event.state & 0x0004):
            self.canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")

            # Update visible range and load new cells
            self._update_lazy_load()
...
class LoggingReviewDialog:
    ...
    def _build_tag_buttons(self):
        ...
        # Clear ALL children of tag_frame (including labels and buttons)
        for widget in self.tag_frame.winfo_children():
            widget.destroy()

        self.tag_buttons = {}

        # Get active tags from manager
        ...
        # Create button for each active tag
        for tag in active_tags:
            # Create button with icon if present
            ...
            btn = ModernButton(
                ...
                text=button_text,
                color=tag.color,
                command=lambda t=tag.id: self._set_mode(t),  # Use same _set_mode as classifications!
                theme_colors=self.gui_manager.theme_colors,
            )
            btn.pack(side=tk.LEFT, padx=2)
    ...
    def _manual_save(self):
        """Save classified images to JSON register"""
        if not self.all_images:
            DialogHelper.show_message(
        ...
        try:
            # Collect all classified images for batch update
            reviews_to_save = []
            for img in self.all_images:
                # Skip unassigned images - check both string and enum forms
                if (
                ...
                # Check classification, comments, AND tags
            ...
            # Update save state only for classified images
            self.last_save_state = {
                img.filename: img.classification
                for img in self.all_images
                if img.classification != ClassificationCategory.UNASSIGNED
            }
            self.has_unsaved_changes = False
            ...
            if self.grid_canvas:
                self.grid_canvas._refresh_classifications()
            self._update_statistics()
            self._update_status(f"Saved {reviews_saved} classifications to register")
    ...
    def _autosave(self):
        ...
        if self.all_images and self.has_unsaved_changes:
            try:
                ...
                reviews_to_save = []
                for img in self.all_images:
                    if img.classification != "Unassigned" and img.classification:
                        # Convert tags set to list for JSON serialization
                        tags_list = list(img.tags) if hasattr(img, "tags") and img.tags else []
                ...
                if self.autosave_path and self.displayed_images:
                    # Create CSVHandler on-demand for export only
                ...
                # Update last save state for ALL images
                self.last_save_state = {
                    img.filename: img.classification for img in self.all_images
                }
            except Exception as e:
    ...
    def _bind_shortcuts(self):
        ...
        # Clear any existing classification/tag keybindings
        ...
        # Get ALL items (classifications + tags) with keybindings
        all_items_with_keys = []
        ...
        # Bind all keybindings (classifications and tags together)
        for key, item_id in all_items_with_keys:
            ...
            self.logger.debug(f"Bound {key} to item {item_id}")
    ...
    def _update_statistics(self):
        ...
        for img in self.displayed_images:
            if img.classification != "Unassigned" and img.classification:
                if self.last_save_state.get(img.filename) == img.classification:
                    saved_count += 1
                else:
                    pending_count += 1
