...
class DrillholeTraceDesigner:
    ...
    def _create_dialog(self):
        ...
        self.dialog = DialogHelper.create_dialog(
            ...
            modal=True,
            # min_width=1400, # NOT REQUIRED
            # min_height=900, # NOT REQUIRED
        )
    ...
    def _create_center_panel(self, parent):
        ...
        ttk.Label(
            orientation_frame, text=DialogHelper.t("Compartment Orientation:")
        ...
        orientation_dropdown = tk.OptionMenu(
            orientation_combo_frame, self.orientation_var, *orientation_choices
        )
    ...
    def _add_data_source_widget(self, filepath: str, status: str):
        ...
        source_frame = ttk.Frame(
            self.data_sources_frame, relief=tk.RIDGE, borderwidth=1
        )
        ...
        name_label = ttk.Label(
            source_frame, text=filename, font=self.gui_manager.fonts["normal"]
        )
        ...
        self.data_source_widgets.append(
            {"filename": filename, "frame": source_frame, "status": status}
        )
    ...
    def _remove_data_source(self, filename: str, frame: ttk.Frame):
        ...
        # Update widget list
        self.data_source_widgets = [
            w for w in self.data_source_widgets if w["filename"] != filename
        ]
            ...
            if "column_combos" in widget_info:
                for combo_info in widget_info["column_combos"]:
                    ...
                    new_dropdown = tk.OptionMenu(
                        combo_frame, column_var, *columns if columns else [""]
                    )
    ...
    def _add_plot_column(self):
        ...
        # Container frame
        plot_frame = ttk.Frame(self.plot_columns_frame, relief=tk.RIDGE, borderwidth=1)
        ...
        ttk.Label(type_frame, text=DialogHelper.t("Plot Type:")).pack(
        ...
        # Type dropdown
        type_dropdown = tk.OptionMenu(type_combo_frame, type_var, *plot_types)
        ...
        ttk.Label(width_frame, text=DialogHelper.t("Width (pixels):")).pack(
            side=tk.LEFT, padx=(0, 10)
        )

        width_var = tk.IntVar(value=100)
        width_entry = self.gui_manager.create_entry_with_validation(
            width_frame, width_var, width=10
        )
        ...
        ttk.Label(color_frame, text=DialogHelper.t("Color Map:")).pack(
            side=tk.LEFT, padx=(0, 10)
        )
        ...
        def configure_colors():
            nonlocal color_map
            # Get selected columns to determine data values and column name
            data_values = []
            data_column = None
            ...
            # Check if dialog returned a color map
            if hasattr(dialog, 'result') and dialog.result:
                color_map = dialog.result
        ...
        type_var.trace_add(
            "write", lambda *args: self._on_plot_type_change(widget_info)
        )
    ...
    def _add_column_selector(self, widget_info: dict):
        ...
        column_dropdown = tk.OptionMenu(
            column_combo_frame, column_var, *columns if columns else [""]
        )
        ...
        if len(widget_info["column_combos"]) > 0:  # Keep at least one
            remove_btn = self.gui_manager.create_modern_button(
                ...
                text="−",
                color=self.gui_manager.theme_colors["accent_red"],
                command=lambda: self._remove_column_selector(
                    widget_info, selector_frame, column_var
                ),
            )
    ...
    def _remove_column_selector(
        self, widget_info: dict, frame: ttk.Frame, var: tk.StringVar
    ):
        """Remove a column selector."""
        frame.destroy()
        widget_info["column_combos"] = [
            c for c in widget_info["column_combos"] if c["var"] != var
        ]
    ...
    def _remove_plot_column(self, frame: ttk.Frame):
        ...
        self.plot_column_widgets = [
            w for w in self.plot_column_widgets if w["frame"] != frame
        ]
    ...
    def _update_preview(self):
        ...
        try:
            ...
            for widget_info in self.plot_column_widgets:
                ...
                # Create plot config
                config = PlotConfig(plot_type=PlotType[plot_type.upper()], width=width)
        ...
        try:
            ...
            for widget_info in self.plot_column_widgets:
                plot_config = {
                    ...
                    "columns": [
                        c["var"].get()
                        for c in widget_info["column_combos"]
                        if c["var"].get()
                    ],
                }
