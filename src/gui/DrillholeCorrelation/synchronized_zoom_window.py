...
class SynchronizedZoomWindow(tk.Toplevel):
    ...
    def add_drillhole(self, hole_id: str, column_widget: Any) -> None:
        ...
        viz_columns = column_widget.viz_columns if getattr(column_widget, "show_data_viz", True) else []
        canvas_width = self.image_width + (len(viz_columns) * self.data_column_width if viz_columns else 0)
    ...
    def refresh_layout(self) -> None:
        """Refresh zoom canvas widths after graph visibility or width settings change."""
        for hole_id, column_widget in self.active_holes.items():
            canvas = self.hole_canvases.get(hole_id)
            if not canvas:
                continue
            viz_columns = column_widget.viz_columns if getattr(column_widget, "show_data_viz", True) else []
            canvas_width = self.image_width + (len(viz_columns) * self.data_column_width if viz_columns else 0)
            canvas.configure(width=canvas_width)
            depth = self.current_depth_by_hole.get(hole_id)
    ...
    def _update_hole_view(self, hole_id: str, center_depth: float) -> None:
        ...
        for i, idx in enumerate(range(start_idx, end_idx)):
            ...
            if getattr(column_widget, "show_data_viz", True) and column_widget.viz_columns:
                self._draw_data_viz(
                    canvas,
                    hole_id,
                    interval,
    ...
    def _draw_enlarged_image(self, canvas: tk.Canvas, hole_id: str, interval: Any, x: int, y: int, is_center: bool) -> None:
        ...
        try:
            ...
            crop_x = max(0, (new_w - self.image_width) // 2)
            crop_y = max(0, (new_h - self.cell_height) // 2)
            resized = resized[
                crop_y:crop_y + self.cell_height,
                crop_x:crop_x + self.image_width
            ]
    ...
    def _draw_data_viz(
        ...
        x: int,
        y: int,
        viz_columns: List[Dict],
        column_widget: Optional[Any] = None,
    ) -> None:
    ...
    def _get_interval_value(
        ...
        """Resolve a data-viz value the same way the main column widget does."""
        if column_widget and hasattr(column_widget, "_get_value_for_interval"):
            value = column_widget._get_value_for_interval(interval, column_name, source_name)
            if value is not None:
                return value

        if not getattr(interval, "csv_data", None):
            return None

        base_column, requested_source = self._parse_column_with_source(column_name)
        requested_source = requested_source or source_name
        requested_lower = column_name.lower().strip()
        base_lower = base_column.lower().strip()

        for key, raw_value in interval.csv_data.items():
            key_text = str(key)
            key_lower = key_text.lower().strip()
            key_base, key_source = self._parse_column_with_source(key_text)
            source_matches = requested_source is None or (
                key_source is not None
                and key_source.lower().strip() == requested_source.lower().strip()
            )

            if (
                key_lower == requested_lower
                or key_lower == base_lower
                or (key_base.lower().strip() == base_lower and source_matches)
            ):
                try:
    ...
    @staticmethod
    def _parse_column_with_source(column_spec: str) -> Tuple[str, Optional[str]]:
        """Parse 'column (source)' into column and source parts."""
        import re
