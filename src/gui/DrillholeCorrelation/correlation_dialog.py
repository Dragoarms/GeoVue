...
from gui.DrillholeCorrelation.trace_processing import canonical_scale_mode
...
class CorrelationDialog(tk.Toplevel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.session = CorrelationSession(
            ...
            viz_columns=config_manager.get(
                "correlation_viz_columns",
                config_manager.get("viz_columns", []),
            )
        )
        self.thickness_profiles: Dict[str, List[Dict[str, float]]] = {}
    ...
    def _create_toolbar(self, parent: tk.Widget) -> None:
        ...
        self.graph_toggle_btn = ModernButton(
            toolbar,
            text=self.translator.translate("Hide Graphs"),
            command=self._toggle_graph_columns,
            color=self.theme_colors["accent_blue"],
            theme_colors=self.theme_colors
        )
        self.graph_toggle_btn.pack(side="left", padx=2, pady=5)
        ...
        ModernButton(
            toolbar,
            text="OTV " + self.translator.translate("Open Televiewer"),
            command=self._open_televiewer_for_current_context,
            color=self.theme_colors["accent_blue"],
            theme_colors=self.theme_colors
        ).pack(side="left", padx=2, pady=5)
    ...
    def _toggle_graph_columns(self) -> None:
        """Collapse or restore data graph columns for all drillhole widgets."""
        self.graphs_collapsed = not self.graphs_collapsed
        show_graphs = not self.graphs_collapsed
        logger.info("%s correlation graph columns", "Showing" if show_graphs else "Hiding")

        for hole_id, widget in self.column_widgets.items():
            if hasattr(widget, "set_data_viz_visible"):
                widget.set_data_viz_visible(show_graphs)
            self._refresh_column_window_size(hole_id)

        if hasattr(self, "graph_toggle_btn"):
            button_text = "Hide Graphs" if show_graphs else "Show Graphs"
            self.graph_toggle_btn.set_text(self.translator.translate(button_text))

        if self.zoom_window_visible and self.zoom_window:
            if hasattr(self.zoom_window, "refresh_layout"):
    ...
    def _load_visible_thumbnails_for_all(self) -> None:
        """Ask drillhole widgets to lazy-load thumbnails in the current viewport."""
        if not getattr(self, "column_widgets", None):
            return

        try:
            visible_top = self.canvas.canvasy(0)
            visible_bottom = self.canvas.canvasy(max(1, self.canvas.winfo_height()))
        except Exception:
            return

        for hole_id, widget in self.column_widgets.items():
            if not hasattr(widget, "load_visible_thumbnails"):
                continue

            _, y_pos = self.column_positions.get(hole_id, (0, 0))
            widget_top = visible_top - y_pos
            widget_bottom = visible_bottom - y_pos

            try:
                total_height = widget.get_total_height() if hasattr(widget, "get_total_height") else 0
                if total_height and widget_bottom < -1000:
                    continue
                if total_height and widget_top > total_height + 1000:
                    continue
                widget.load_visible_thumbnails(widget_top, widget_bottom)
            except Exception as e:
                logger.debug(f"Could not refresh visible thumbnails for {hole_id}: {e}")
    ...
    def add_hole(self, hole_id: str, y_offset: float = 0) -> None:
        ...
        try:
            ...
            column_widget = DrillholeColumnWidget(
                ...
                viz_columns=self._prepare_viz_columns_for_display(self.session.viz_columns)
            )
            if self.graphs_collapsed:
                column_widget.set_data_viz_visible(False)
            ...
            column_widget.on_televiewer_open_requested = self._handle_televiewer_open_request
            column_widget.on_layout_changed = lambda hid=hole_id: self._handle_column_layout_changed(hid)
            ...
            # Recalculate global trace ranges now the new hole is available.
    ...
    def _bind_column_drag(self, hole_id: str, widget: DrillholeColumnWidget) -> None:
        ...
        def on_body_press(event):
            if (
                hasattr(widget, "is_on_data_column_resize_handle")
                and widget.is_on_data_column_resize_handle(event.x, event.y)
            ):
                return
    ...
    def _prepare_viz_columns_for_display(
        ...
        """Copy viz configs and attach global ranges where requested."""
        display_columns = [dict(config) for config in (viz_columns or [])]
        if not display_columns or not self.column_widgets:
            return display_columns

        for config in display_columns:
            if canonical_scale_mode(config.get("scale_mode", "raw")) != "global":
                continue
            ...
            for widget in self.column_widgets.values():
                if hasattr(widget, "get_viz_raw_values"):
                    values.extend(widget.get_viz_raw_values(column_name, source_name))

            if values:
                config["global_min_value"] = min(values)
                config["global_max_value"] = max(values)

        return display_columns

    def _refresh_viz_columns_for_all_widgets(self) -> None:
        """Reapply session viz settings to every drillhole widget."""
    ...
    def _refresh_column_window_size(self, hole_id: str) -> None:
        ...
        if not widget or not window_id:
            return
    ...
    def _refresh_all_column_window_sizes(self) -> None:
        ...
        for hole_id in list(self.column_widgets):
            self._refresh_column_window_size(hole_id)

    def _handle_column_layout_changed(self, hole_id: str) -> None:
        """Handle a child drillhole widget changing its rendered dimensions."""
        self._refresh_column_window_size(hole_id)
    ...
    def remove_hole(self, hole_id: str) -> None:
        ...
        # Recalculate global trace ranges after removing the hole.
        self._refresh_viz_columns_for_all_widgets()
    ...
    def _open_televiewer_for_current_context(self) -> None:
        """Open the Televiewer viewer for the currently selected or hovered hole."""
        hole_id, depth = self._televiewer_context()
        if not hole_id:
            self.dialog_helper.show_info(
                self.translator.translate("No Holes"),
                self.translator.translate("No holes are loaded."),
            )
            return
        self._open_televiewer_for_context(hole_id, depth)

    def _handle_televiewer_open_request(self, hole_id: str, depth: float) -> None:
        """Open Televiewer from a drillhole column context-menu request."""
        self._open_televiewer_for_context(hole_id, depth)

    def _open_televiewer_for_context(self, hole_id: str, depth: Optional[float] = None) -> None:
        """Launch the processed Televiewer dataset for one hole/depth context."""
        try:
            from gui.Televiewer.televiewer_viewer_dialog import open_televiewer_for_hole

            start_depth = self._televiewer_start_depth(depth)
            open_televiewer_for_hole(
                self,
                self.gui_manager,
                self.file_manager,
                config_manager=self.config_manager,
            ...
            self.status_var.set(
                f"Opened Televiewer launcher for {hole_id}"
                + (f" near {float(depth):.2f} m" if depth is not None else "")
            )
        except Exception as exc:
            logger.error(f"Error opening Televiewer for {hole_id}: {exc}", exc_info=True)
            self.dialog_helper.show_error(
                self.translator.translate("Televiewer Viewer Error"),
                str(exc),
            )

    def _televiewer_context(self) -> Tuple[Optional[str], Optional[float]]:
        if self.selected_region:
            hole_id, depth_from, depth_to = self.selected_region
            return hole_id, (float(depth_from) + float(depth_to)) / 2.0

        hover_hole = getattr(self, "_last_hover_hole", None)
        if hover_hole in self.column_widgets:
            depth = self._depth_for_column_canvas_y(hover_hole, getattr(self, "_last_canvas_y", None))
            return hover_hole, depth
        ...
        if self.column_widgets:
            ...
            hole_id = sorted_holes[0]
            return hole_id, self._first_depth_for_hole(hole_id)
        return None, None

    def _depth_for_column_canvas_y(self, hole_id: str, canvas_y: Any) -> Optional[float]:
        if canvas_y is None or hole_id not in self.column_widgets:
            return None
        try:
            return self.column_widgets[hole_id]._canvas_y_to_depth(float(canvas_y))
        except Exception:
            logger.debug("Could not resolve Televiewer context depth", exc_info=True)
            return None

    def _first_depth_for_hole(self, hole_id: str) -> Optional[float]:
        widget = self.column_widgets.get(hole_id)
        intervals = getattr(widget, "intervals", None) if widget else None
        if not intervals:
            return None
        try:
            return float(min(interval.depth_from for interval in intervals))
        except Exception:
            return None

    @staticmethod
    def _televiewer_start_depth(depth: Optional[float]) -> Optional[float]:
        if depth is None:
            return None
        try:
            return max(0.0, float(depth) - 1.0)
    ...
    def _show_viz_settings(self) -> None:
        ...
        try:
            ...
            if self.column_widgets and hasattr(self.data_manager, "get_correlation_source_names_for_holes"):
                allowed_sources = set(
                    self.data_manager.get_correlation_source_names_for_holes(
                        list(self.column_widgets.keys())
                    )
                )
                logger.debug(f"Correlation viz source filter: {sorted(allowed_sources)}")
    ...
    def _show_stretch_dialog(self) -> None:
        ...
        default_to = 10.0
        if self.selected_region:
            selected_hole, depth_from, depth_to = self.selected_region
            default_from = float(depth_from)
            default_to = float(depth_to)
        elif self.column_widgets:
            selected_hole = next(iter(self.column_widgets))
            intervals = self.column_widgets[selected_hole].intervals
            if intervals:
                default_from = float(intervals[0].depth_from)
                default_to = min(float(intervals[-1].depth_to), default_from + 10.0)
        ...
        tk.Label(
            top_row,
            text=self.translator.translate("Hole"),
            bg=self.theme_colors["background"],
            fg=self.theme_colors["text"],
            font=self.fonts["small"],
        ).pack(side="left", padx=(0, 8))
        if hole_ids:
            ttk.OptionMenu(top_row, hole_var, hole_var.get(), *hole_ids).pack(side="left")
        else:
            tk.Label(
                top_row,
        ...
        status_label = tk.Label(
            ...
            bg=self.theme_colors["background"],
            fg=self.theme_colors["accent_yellow"],
            font=self.fonts["small"],
            anchor="w",
            justify="left",
        )
        ...
        def render_rows() -> None:
            ...
            for col, header in enumerate(headers):
                tk.Label(
                    table_frame,
                    text=self.translator.translate(header),
                    bg=self.theme_colors["background"],
                    fg=self.theme_colors["subtext"],
                    font=self.fonts["small"],
