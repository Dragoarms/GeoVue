...
class DrillholeSelectionDialog(tk.Toplevel):
    ...
    def __init__(self, parent, collar_data: pd.DataFrame,
        ...
        Args:
            ...
            data_manager: Optional DataCoordinator for assay availability status
        """
        ...
        # Data
        self.collar_data = self._add_assay_status_to_collar_data(collar_data.copy())
    ...
    def _add_assay_status_to_collar_data(self, collar_df: pd.DataFrame) -> pd.DataFrame:
        """Add has_assays boolean status used by the minimap colors."""
        if collar_df.empty or "holeid" not in collar_df.columns:
            collar_df["has_assays"] = False
            return collar_df

        holes_with_assays: Set[str] = set()
        if self.data_manager and hasattr(self.data_manager, "get_holes_with_assays"):
            try:
                holes_with_assays = {
                    str(hole_id).strip().upper()
                    for hole_id in self.data_manager.get_holes_with_assays()
                    if str(hole_id).strip()
                }
            except Exception as e:
                self.logger.warning(f"Could not determine assay status for minimap: {e}")

        collar_df["has_assays"] = (
            collar_df["holeid"].astype(str).str.strip().str.upper().isin(holes_with_assays)
        )
        return collar_df
    ...
    def _create_minimap_panel(self, parent):
        ...
        # Load collar data
        self.logger.debug("Loading collar data into minimap")
        self.minimap.set_collar_data(self.collar_data)
        self.logger.debug("Collar data loaded")
        
        # Force immediate zoom to fit and render
        self.logger.debug(
            "Minimap has %s collars",
            len(self.minimap.collar_data) if hasattr(self.minimap, 'collar_data') else 0,
        )
        self.logger.debug(
            "Map bounds: X(%.1f to %.1f), Y(%.1f to %.1f)",
            self.minimap.map_min_x,
            self.minimap.map_max_x,
            self.minimap.map_min_y,
            self.minimap.map_max_y,
        )
        ...
        self.logger.debug("After zoom_to_fit: zoom_level=%s", self.minimap.zoom_level)
        self.logger.debug("Canvas size: %sx%s", self.minimap.winfo_width(), self.minimap.winfo_height())
        # Controls
