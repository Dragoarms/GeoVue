...
if threading.current_thread() != threading.main_thread():
    raise RuntimeError("❌ EmbeddingTrainingDialog called from a background thread!")
from gui.dialog_helper import DialogHelper
from processing.MachineLearningStep.embedding_trainer import generate_embeddings, plot_embeddings
...
class EmbeddingTrainingDialog:
    ...
    def _create_dialog(self) -> None:
        ...
        self.gui_manager.create_modern_button(
            btn_row,
            text=DialogHelper.t("Select Image Folder"),
        ...
        self.gui_manager.create_modern_button(
            btn_row,
            text=DialogHelper.t("Select CSV File"),
            color=self.gui_manager.theme_colors["accent_blue"],
        ...
        checkbox_outer = ttk.Frame(frame)
        checkbox_outer.pack(fill=tk.BOTH, expand=True)
        ...
        scrollbar = ttk.Scrollbar(checkbox_outer, orient="vertical", command=canvas.yview)
        canvas.configure(yscrollcommand=scrollbar.set)
        canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        self.checkbox_frame = ttk.LabelFrame(canvas, text=DialogHelper.t("Select Input Columns"))
        window = canvas.create_window((0, 0), window=self.checkbox_frame, anchor="nw")

        def _update_scroll(event):
            canvas.configure(scrollregion=canvas.bbox("all"))
            canvas.itemconfigure(window, width=event.width)

        self.checkbox_frame.bind("<Configure>", _update_scroll)

        self.gui_manager.create_modern_button(
            frame,
            text=DialogHelper.t("Generate Embeddings"),
    ...
    def _process(self) -> None:
        ...
        os.close(fd)
        plot_embeddings(embeddings, temp_path, gui_manager=self.gui_manager)
        self.file_manager.save_embedding_plot(temp_path)
        os.remove(temp_path)
