...
class CSVExportDialog:
    ...
    # Display names for RC metrics columns
    RC_METRICS_DISPLAY_NAMES = OrderedDict([
        ("weighted_hardness", "Weighted Hardness"),
        ("no_hardness_pct", "No Hardness %"),
        ("total_logged_pct", "Total Logged %"),
        ("normalisation_factor", "Normalisation Factor"),
        ("total_gangue_pct", "Total Gangue %"),
        ("si_gangue_pct", "Si Gangue %"),
        ("al_gangue_pct", "Al Gangue %"),
        ("carbonate_gangue_pct", "Carbonate Gangue %"),
        ("sulphide_gangue_pct", "Sulphide Gangue %"),
        ("manganese_gangue_pct", "Manganese Gangue %"),
        ("mafics_gangue_pct", "Mafics Gangue %"),
        ("undefined_gangue_pct", "Undefined Gangue %"),
        ("magnesium_gangue_pct", "Magnesium Gangue %"),
        ("magnetite_pct", "Magnetite %"),
        ("si_friable_pct", "Si Friable %"),
        ("si_non_friable_pct", "Si Non-Friable %"),
        ("zonation_un_pct", "Zonation Un %"),
        ("zonation_pr_pct", "Zonation Pr %"),
        ("zonation_de_pct", "Zonation De %"),
        ("zonation_hy_pct", "Zonation Hy %"),
        ("zonation_hc_pct", "Zonation Hc %"),
        ("weighted_zonation", "Weighted Zonation"),
        ("quartz_pct", "Quartz %"),
        ("chert_pct", "Chert %"),
    ])
    ...
    def __init__(
        ...
        self.rc_metrics_column_vars: Dict[str, tk.BooleanVar] = {}
    ...
    def show(self) -> Optional[str]:
        ...
        self._populate_rc_metrics_columns()
        ...
        logger.info(f"rc_metrics_column_vars: {len(self.rc_metrics_column_vars)} entries")
    ...
    def _create_column_selection(self, parent):
        ...
        # Add RC metrics columns
        self._create_rc_metrics_columns_group(scroll_frame)
    ...
    def _create_rc_metrics_columns_group(self, parent):
        """Create placeholder frame for RC metrics columns (populated after dialog creation)."""
        self.rc_metrics_frame = ttk.LabelFrame(parent, text="RC Metrics (Calculated)", padding=5)
        self.rc_metrics_frame.pack(fill=tk.X, pady=2, padx=2)
        
        self.rc_metrics_loading_label = ttk.Label(
            self.rc_metrics_frame,
            text="Loading RC metrics...",
            style="Content.TLabel"
        )
        self.rc_metrics_loading_label.pack(anchor=tk.W)
    
    def _populate_rc_metrics_columns(self):
        """Populate RC metrics columns from RCMetricsStore via DataCoordinator."""
        logger.info("Populating RC metrics columns")
        
        self.rc_metrics_loading_label.destroy()
        
        # Check if RC metrics are available
        if not self.data_coordinator or not self.data_coordinator.has_rc_metrics:
            logger.info("No RC metrics available")
            ttk.Label(
                self.rc_metrics_frame,
                text="No RC metrics computed",
                style="Content.TLabel"
            ).pack(anchor=tk.W)
            return
        
        rc_store = self.data_coordinator.rc_metrics_store
        metrics_df = rc_store.get_metrics_dataframe()
        if metrics_df is None or metrics_df.empty:
            logger.info("RC metrics DataFrame is empty")
            ttk.Label(
                self.rc_metrics_frame,
                text="No RC metrics data available",
                style="Content.TLabel"
            ).pack(anchor=tk.W)
            return
        
        # Skip key/index columns
        skip_cols = {'hole_id', 'depth_from', 'depth_to', '_hole_upper', '_depth_int'}
        
        # Build checkboxes in a grid layout
        col_container = ttk.Frame(self.rc_metrics_frame)
        col_container.pack(fill=tk.X, padx=(10, 0))
        ...
        for col_id, display_name in self.RC_METRICS_DISPLAY_NAMES.items():
            if col_id in skip_cols:
                continue
            # Only show columns that actually exist in the metrics DataFrame
            if col_id not in metrics_df.columns:
                continue
            
            rc_col_id = f"rc_{col_id}"
            var = tk.BooleanVar(value=False)  # Off by default
            self.rc_metrics_column_vars[rc_col_id] = var
        ...
        if columns_added > 0:
            # Add select all/none buttons for RC metrics
            rc_btn_frame = ttk.Frame(self.rc_metrics_frame)
            rc_btn_frame.pack(fill=tk.X, pady=(5, 0))
            
            ttk.Button(
                rc_btn_frame,
                text="Select All RC Metrics",
                command=lambda: self._set_all_rc_metrics_columns(True),
                style="Accent.TButton"
            ).pack(side=tk.LEFT, padx=(0, 5))
            
            ttk.Button(
                rc_btn_frame,
                text="Deselect All RC Metrics",
                command=lambda: self._set_all_rc_metrics_columns(False),
            ).pack(side=tk.LEFT)
        
        stats = rc_store.get_statistics()
        logger.info(f"Populated {columns_added} RC metrics columns ({stats.get('total_intervals', 0)} intervals)")
    
    def _set_all_rc_metrics_columns(self, value: bool):
        """Set all RC metrics column checkboxes to value."""
        logger.info(f"Setting all {len(self.rc_metrics_column_vars)} RC metrics columns to {value}")
        for col_id, var in self.rc_metrics_column_vars.items():
            var.set(value)
    ...
    def _get_selected_columns(self) -> List[str]:
        ...
        for col_id, var in self.rc_metrics_column_vars.items():
            if var.get():
                selected.append(col_id)
                rc_selected += 1
                logger.debug(f"  RC metrics column selected: {col_id}")
        logger.debug(f"RC metrics columns: {rc_selected} selected from {len(self.rc_metrics_column_vars)} total")
    ...
    def _get_filtered_images(self) -> List:
        ...
        if filter_mode == "all":
            ...
            if self.displayed_images is not None:
                result = self.displayed_images
            else:
                result = self.all_images
                logger.warning("No displayed_images provided, falling back to all_images")
        else:
            result = self.all_images
    ...
    def _build_export_dataframe(self, images: List, columns: List[str]) -> pd.DataFrame:
        ...
        # =================================================================
        # STEP 2b: Merge RC metrics data
        # =================================================================
        rc_cols = [c for c in columns if c.startswith('rc_')]
        
        if rc_cols and self.data_coordinator and self.data_coordinator.has_rc_metrics:
            logger.info(f"Step 2b: Merging {len(rc_cols)} RC metrics columns...")
            
            rc_store = self.data_coordinator.rc_metrics_store
            metrics_df = rc_store.get_metrics_dataframe()
            
            if metrics_df is not None and not metrics_df.empty:
                # Reset index to avoid ambiguity (_hole_upper/_depth_int are both index and columns)
                if isinstance(metrics_df.index, pd.MultiIndex):
                    metrics_df = metrics_df.reset_index(drop=True)
                
                # Map rc_col_id -> actual metrics column name (strip 'rc_' prefix)
                rc_base_cols = []
                rc_rename = {}
                for rc_col_id in rc_cols:
                    base_col = rc_col_id[3:]  # Strip 'rc_' prefix
                    if base_col in metrics_df.columns:
                        rc_base_cols.append(base_col)
                        rc_rename[base_col] = rc_col_id
                
                if rc_base_cols:
                    # Select only needed columns + merge keys
                    merge_cols = ['_hole_upper', '_depth_int'] + rc_base_cols
                    rc_merge_df = metrics_df[merge_cols].drop_duplicates(
                        subset=['_hole_upper', '_depth_int'], keep='first'
                    )
                    
                    df = df.merge(
                        rc_merge_df,
                        on=['_hole_upper', '_depth_int'],
                        how='left',
                        suffixes=('', '__rc')
                    )
                    
                    # Rename base cols to rc_ prefixed names
                    df = df.rename(columns=rc_rename)
                    
                    # Fill NaN
                    for rc_col_id in rc_cols:
                        if rc_col_id in df.columns:
                            df[rc_col_id] = df[rc_col_id].fillna("")
                        else:
                            df[rc_col_id] = ""
                    ...
                    logger.info(f"  Merged {len(rc_base_cols)} RC metrics columns")
    ...
    def _deselect_all_columns(self):
        ...
        for var in self.csv_column_vars.values():
            var.set(False)
        for var in self.rc_metrics_column_vars.values():
            var.set(False)
        self._update_preview()
