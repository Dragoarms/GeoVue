...
class DialogHelper:
    ...
    def _check_main_thread(method_name):
        ...
        logger.debug(
            f"DialogHelper.{method_name} called from thread: {current_thread.name} (main={is_main_thread})"
        )
        ...
        if not is_main_thread:
            ...
            logger.error(
                f"⚠️ THREAD SAFETY ERROR: DialogHelper.{method_name} called from non-main thread!\n"
    ...
    def create_dialog(parent, title, modal=True, topmost=True):
        ...
        if DialogHelper.gui_manager and hasattr(
            DialogHelper.gui_manager, "theme_colors"
        ):
    ...
    def create_error_dialog(parent, title, message):
        ...
        dialog = DialogHelper.create_dialog(parent, title, modal=True, topmost=True)
        ...
        frame = tk.Frame(
            dialog, bg=theme_colors["background"] if theme_colors else None
        )
        ...
        message_label = tk.Label(
            ...
            font=(
                DialogHelper.gui_manager.fonts["normal"]
        ...
        if DialogHelper.gui_manager and theme_colors:
            ...
            ok_button = ttk.Button(
                dialog, text=DialogHelper.t("OK"), command=dialog.destroy
            )
            ok_button.pack(pady=(0, 20))
    ...
    def create_message_dialog(parent, title, message, button_text="OK"):
        ...
        if DialogHelper.gui_manager and theme_colors:
            ...
            # Fallback to ttk button
            ok_button = ttk.Button(
                dialog, text=DialogHelper.t(button_text), command=dialog.destroy
            )
            ok_button.pack(pady=(0, 20))
        ...
        dialog.update_idletasks()
        DialogHelper.center_dialog(
            dialog, parent, size_ratio=0.4, min_width=300, min_height=150
        )
    ...
    def show_message(parent, title, message, message_type="info"):
        ...
        msg_label = ttk.Label(top_frame, text=message, wraplength=400, justify=tk.LEFT)
    ...
    def confirm_dialog(parent, title, message, yes_text="Yes", no_text="No"):
        ...
        screen_width = dialog.winfo_screenwidth()
        message_width = min(
            400, int(screen_width * 0.3)
        )  # Cap at 400px or 30% of screen width

        msg_label = ttk.Label(
            main_frame, text=message, wraplength=message_width, justify=tk.CENTER
        )
        ...
        # Position after layout is complete
        dialog.after_idle(
            lambda: DialogHelper._position_dialog_on_parent(dialog, parent)
        )
    ...
    def handle_rejection(
        parent_dialog, image_path, metadata_callback=None, cleanup_callback=None
    ):
        """
        ...
        Note: This method does NOT handle file operations. The actual file movement of the original full sized image
        ...
        try:
            ...
            if not all([hole_id, depth_from is not None, depth_to is not None]):
                DialogHelper.show_message(
                    ...
                    DialogHelper.t(
                        "Please enter Hole ID and depth information before rejecting"
                    ),
                    message_type="error",
                )
                return None
            ...
            rejection_dialog = DialogHelper.create_dialog(
                parent_dialog, "Confirm Rejection", modal=True, topmost=True
            )
            ...
            info_label = ttk.Label(
                main_frame, text=info_text, wraplength=450, justify=tk.CENTER
            )
            info_label.pack(pady=(0, 15))

            # Reason frame
            reason_frame = ttk.LabelFrame(
                main_frame, text=DialogHelper.t("Reason for Rejection"), padding="10"
            )
            ...
            scrollbar = ttk.Scrollbar(
                text_frame, orient="vertical", command=reason_text.yview
            )
            ...
            if gui_manager and theme_colors:
                ...
                reject_btn = ttk.Button(
                    button_frame, text=DialogHelper.t("Reject"), command=on_reject
                )
                cancel_btn = ttk.Button(
                    button_frame, text=DialogHelper.t("Cancel"), command=on_cancel
                )
