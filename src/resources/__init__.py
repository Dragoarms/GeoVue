...
def get_excel_template_path():
    ...
    return pkg_resources.files(__package__) / "Register Template File" / "Chip_Tray_Register.xltx"
...
def get_color_preset_path(preset_name: str) -> Path:
    ...
    possible_paths = [
        ...
        Path(sys.executable).parent / "_internal" / "resources" / "color_presets" / preset_name,
    ]
...
def list_color_presets() -> list:
    ...
    try:
        ...
        if color_presets_dir.is_dir():
            presets = [f.name for f in color_presets_dir.iterdir() if f.name.endswith('.json')]
            if presets:
                return presets
    except Exception:
        pass
