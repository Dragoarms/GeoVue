...
class PhotoRenamerGUI:
    ...
    def _build_gui(self):
        ...
        self.root.bind("<Left>", lambda e: self._previous_photo())

        # Bind validation check on entry change
        self.hole_id_entry.bind("<KeyRelease>", lambda e: self._check_validation())
    ...
    def _increment_hole(self):
        ...
        to 20m (first interval). Follows PEP-8 guidelines.
        """
        import re  # local import to avoid changing module imports
    ...
    def _rename_and_next(self):
        ...
        if not hole_id or not to_str:
            messagebox.showwarning("Missing Data", "Please enter hole ID and to depth")
            return
        ...
        try:
            ...
            # Format as integers if whole numbers, otherwise one decimal place
            from_str_fmt = (
                f"{int(from_depth)}"
