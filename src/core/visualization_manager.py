...
"""Roles and Responsibilities of VisualizationManager
1. Primary Purpose
    The VisualizationManager serves as a centralized image version control system that tracks all image transformations throughout the processing pipeline, maintaining relationships between different versions and enabling coordinate scaling between them.

2. Core Responsibilities

A. Image Version Management

    Stores multiple image versions: Original, working (downsampled), oriented, corrected, etc.
    Tracks parent-child relationships between versions
    Manages version lifecycle with automatic cleanup (configurable max versions)
    Provides version access methods: get_version(), get_current_working_image(), get_original_image()

B. Transformation Tracking

    Records all transformations applied to images using ImageTransform objects
    Maintains transformation history with timestamps and parameters
    Supports transformation types: RESIZE, ROTATE, SKEW_CORRECT, etc.
    Enables transformation replay on different image versions

C. Scale Relationship Management

    Tracks scale factors between all image versions bidirectionally
    Calculates transitive scales when direct relationships don't exist
    Updates scale relationships automatically when creating new versions
    Provides coordinate scaling between any two versions via scale_coordinates()

D. Coordinate System Management

    Scales coordinates between different image versions (points, bounding boxes, numpy arrays)
    Handles multiple coordinate formats transparently
    Maintains coordinate accuracy across transformations
    Supports display-to-image coordinate conversion

E. Visualization Creation and Caching

    Creates visualizations via create_visualization() with external draw functions
    Implements smart caching of visualization results (configurable)
    Manages cache invalidation when base versions change
    Optionally stores visualizations as versions

F. Image Processing Operations

    Downsampling: Creates working copies with load_image() (target: 1MP)
    Rotation: Applies rotation transformations with matrix tracking
    Skew correction: Corrects image orientation based on ArUco markers
    Perspective correction: Applies perspective transformations
    Scale estimation: Estimates physical scale from ArUco markers

3. Key Internal State

    versions: Dictionary of all image versions
    scale_relationships: Bidirectional scale mappings between versions
    viz_cache: Cached visualization results
    processing_metadata: General processing information
    original_key, working_key: Quick access to important versions

4. Memory Management

    Automatic version cleanup when exceeding max_versions (default: 5)
    Protected versions: Original and working versions never deleted
    Memory usage tracking via _get_total_memory_usage()
    Cache management with configurable caching

5. ArUco-Specific Features

    Scale estimation from markers (estimate_scale_from_markers())
    Marker quality validation (edge variance, squareness checks)
    Physical measurement calculation (pixels per cm)
    Orientation detection from marker patterns

6. Integration Architecture

    Standalone design: No dependencies on other app components
    External logger support: Uses provided logger or creates own
    Configuration flexibility: Accepts external config overrides
    Clear API boundaries: Well-defined public methods

7. Transformation Flow

    The typical workflow:

        Load original image → creates "original" version
        Downsample if needed → creates "working" version with scale relationship
        Apply transformations → creates new versions with inheritance
        Create visualizations → uses cached or generates new
        Scale coordinates as needed between versions

8. Key Design Patterns

    Version branching: Each transformation creates a new version
    Scale inheritance: New versions inherit parent's scale relationships
    Lazy evaluation: Visualizations created on demand
    Immutability: Original versions never modified, only new versions created

    The VisualizationManager acts as a comprehensive image processing state manager.
    It ensures all transformations are tracked, reversible, and scalable between different resolutions.
    It maintains efficient memory usage through smart caching and cleanup strategies."""
...
class VisualizationManager:
    ...
    def load_image(self, image: np.ndarray, image_path: str) -> None:
        ...
        self.logger.info(f"Loaded original image: shape={image.shape}, dtype={image.dtype}, path={image_path}")
    ...
    def create_working_copy(self, target_pixels: int = 2000000) -> np.ndarray:
        ...
        self.logger.debug(f"Creating working copy: original size={w}x{h} ({original_pixels} pixels)")
        ...
        if original_pixels > target_pixels:
            ...
            working_image = cv2.resize(original, (new_width, new_height), interpolation=cv2.INTER_AREA)
            ...
            self.logger.info(f"Created working copy: {new_width}x{new_height}, scale={scale:.4f}")
        else:
            ...
            self.logger.info("Original image used as working copy (no downsample needed)")
    ...
    def apply_rotation(self, version_key: str, angle: float, 
        ...
        if rotation_matrix is None:
            ...
            self.logger.debug(f"Calculated rotation matrix for angle={angle}, center={center}")
        else:
        ...
        self.logger.info(f"Applied rotation to {version_key}: {angle:.2f}° -> {new_key}")
    ...
    def apply_transform_to_original(self, transform_from_working: Dict[str, Any]) -> np.ndarray:
        """
    ...
    def scale_coordinates(self, coords: Any, from_version: str, to_version: str) -> Any:
        ...
        elif isinstance(coords, list) and coords and \
             all(isinstance(c, (list, tuple)) and len(c) == 4 for c in coords):
            # List of bounding boxes
            return [self.scale_coordinates(c, from_version, to_version) for c in coords]
        
        elif isinstance(coords, list) and coords and \
             all(isinstance(c, (list, tuple)) and len(c) == 2 for c in coords):
            # List of points
    ...
    def create_visualization(self, base_version: str, viz_name: str, 
                           draw_function: Callable, **kwargs) -> np.ndarray:
        """
        ...
        if kwargs.get("save_as_version", False):
            transform = ImageTransform(
                ...
                timestamp=datetime.now(),
                parameters={"draw_function": draw_function.__name__, "kwargs": str(kwargs)},
    ...
    def log_current_state(self) -> None:
        ...
        self.logger.debug(f"Total memory usage: {self._get_total_memory_usage() / 1024 / 1024:.2f} MB")
        self.logger.debug("==================================")
    

    # ==================== Methods using results from arUco_manager ====================

    def _fix_major_orientation(self, image: np.ndarray, markers: Dict[int, np.ndarray],
                            corner_ids: List[int], compartment_ids: List[int]) -> Tuple[np.ndarray, np.ndarray, float]:
        """
        ...
        comp_markers = [(mid, markers[mid]) for mid in compartment_ids 
                        if mid in markers and mid != 24]  # Exclude metadata
    ...
    def correct_image_skew(self, markers: Dict[int, np.ndarray], 
                        marker_config: Dict[str, List[int]] = None,
                        version_key: str = None) -> Dict[str, Any]:
        """
        ...
        oriented_image, orientation_matrix, orientation_angle = self._fix_major_orientation(
        ...
        if orientation_corrected:
            self.logger.info(f"Image orientation was corrected by {orientation_angle:.2f} degrees")
            ...
            self.versions[oriented_key] = ImageVersion(
                ...
                scale_factor_from_original=self.versions[version_key].scale_factor_from_original
            )
    ...
    def _calculate_skew_angle(self, markers: Dict[int, np.ndarray],
                            compartment_ids: List[int]) -> float:
        """
        ...
        if len(centers) < 4:
            self.logger.warning(f"Only {len(centers)} compartment markers for skew estimation")
            return 0.0
    ...
    def rotate_image_to_level(self, rotation_angle: float) -> None:
        """
        Rotate both original and working images to level.
        ...
        # Recreate working copy with rotated image
        if self.working_image is not None:
            self.create_working_copy()

    def _create_oriented_square(self, center: np.ndarray, size: float, 
    ...
    def _create_axis_aligned_square(self, center: np.ndarray, size: float) -> np.ndarray:
        """
    ...
    def _update_scale_relationship(self, from_version: str, to_version: str, scale: float) -> None:
        """
        ...
        self.logger.debug(f"Scale relationship: {from_version} → {to_version} = {scale:.6f} " + 
        ...
        self.logger.debug(f"Inverse relationship: {to_version} → {from_version} = {inverse_scale:.6f}")
    ...
    def _get_scale_factor(self, from_version: str, to_version: str) -> float:
        ...
        if (from_version, to_version) in self.scale_relationships:
            ...
            self.logger.debug(f"Retrieved direct scale: {from_version} → {to_version} = {scale:.6f}")
            return scale
        
        # Try to find path through original
        if (from_version, self.original_key) in self.scale_relationships and \
        (self.original_key, to_version) in self.scale_relationships:
            ...
            self.logger.debug(f"Transitive scale ({from_version} → {self.original_key} → {to_version}): "
        ...
        self.logger.debug(f"Calculated average scale: x={scale_x:.6f}, y={scale_y:.6f}, avg={scale:.6f}")
    ...
    def _inherit_scale_relationships(self, new_version: str, parent_version: str) -> None:
    ...
    def _verify_rotation_matrix(self, matrix: np.ndarray, image_size: Tuple[int, int], 
        ...
        expected_tx = expected_center[0] - cos_theta * expected_center[0] + sin_theta * expected_center[1]
        expected_ty = expected_center[1] - sin_theta * expected_center[0] - cos_theta * expected_center[1]
    ...
    def _cleanup_old_versions(self) -> None:
        ...
        for _, key in removable_versions[:num_to_remove]:
            ...
            self.scale_relationships = {
                k: v for k, v in self.scale_relationships.items()
    ...
    def get_display_image(self, version_key: str, max_width: int, max_height: int) -> np.ndarray:
        """
        ...
        if scale < 1.0:
            ...
            return cv2.resize(image, (display_w, display_h), interpolation=cv2.INTER_AREA)
        
        # Return copy to prevent modifications
        return image.copy()
    
    def display_to_image_coords(self, display_coords: Tuple[int, int], 
                               version_key: str, 
...
class CoordinateMapper:
    ...
    def image_rect_to_canvas(self, x1: float, y1: float, x2: float, y2: float) -> Tuple[float, float, float, float]:
    ...
    def canvas_rect_to_image(self, x1: float, y1: float, x2: float, y2: float) -> Optional[Tuple[float, float, float, float]]:
    ...
    def get_visible_image_region(self) -> Tuple[int, int, int, int]:
        ...
        br = self.canvas_to_image(self.canvas_width, self.canvas_height) or (self.image_width, self.image_height)
    ...
    def get_display_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "display_size": (int(self.image_width * self.scale), int(self.image_height * self.scale))
        }