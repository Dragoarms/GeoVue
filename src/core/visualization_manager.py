...
class VisualizationManager:
    ...
    def create_working_copy(self, target_pixels: int = 2000000) -> np.ndarray:
        ...
        if original_pixels > target_pixels:
            ...
            # Store scale relationships
            self._update_scale_relationship(self.working_key, self.original_key, scale)
            self._update_scale_relationship(self.original_key, self.working_key, 1.0 / scale)
    ...
    def apply_transform_to_original(self, transform_from_working: Dict[str, Any]) -> np.ndarray:
        ...
        if transform_type == "rotation":
            ...
            # Scale crop coordinates from working to original
            working_crop = transform_from_working["crop_region"]  # (x1, y1, x2, y2)
            original_crop = self.scale_coordinates(
                working_crop, 
                self.working_key, 
                self.original_key
            )
            return self._apply_crop(self.original_key, original_crop)
            
        elif transform_type == "perspective":
            # Scale perspective points from working to original
            working_points = transform_from_working["src_points"]
            original_points = self.scale_coordinates(
                working_points,
                self.working_key,
                self.original_key
            )
            return self._apply_perspective(
                self.original_key, 
                original_points,
                transform_from_working["dst_points"]
            )
    ...
    def correct_marker_geometry(self, markers: Dict[int, np.ndarray],
                            ...
                            preserve_orientation: bool = True) -> Dict[int, np.ndarray]:
        """
    ...
    def estimate_scale_from_markers(self, markers: Dict[int, np.ndarray],
        ...
        self.logger.info(f"Scale estimation: {final_scale_px_per_cm:.2f} px/cm (confidence: {confidence:.2%})")
    ...
    def _measure_marker_for_scale(self, corners: np.ndarray, physical_size_cm: float,
        ...
        edge_cv = np.std(edge_array) / np.mean(edge_array) if np.mean(edge_array) > 0 else 1.0
        ...
        if measurements['valid'] and len(diagonal_lengths) == 2:
            if min(diagonal_lengths) > 0:
                ...
                if diagonal_ratio > 1.02:
                    measurements['rejection_reason'] = f"Non-square (diagonal ratio={diagonal_ratio:.2f})"
                    measurements['valid'] = False
    ...
    def _get_scale_factor(self, from_version: str, to_version: str) -> float:
        ...
        if (from_version, to_version) in self.scale_relationships:
            return self.scale_relationships[(from_version, to_version)]
    ...
    def _apply_crop(self, version_key: str, crop_region: Tuple[int, int, int, int]) -> np.ndarray:
    ...
    def _apply_perspective(self, version_key: str, src_points: np.ndarray, 
