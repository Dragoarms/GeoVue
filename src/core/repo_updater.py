...
class RepoUpdater:
    ...
    def __init__(self, 
        ...
        if match:
            ...
            self.logger.info(f"Extracted GitHub info - Owner: {self.owner}, Repo: {self.repo}")
        else:
            self.owner = None
            self.repo = None
    ...
    def get_github_version(self) -> str:
            ...
            try:
                if not self.owner or not self.repo:
                    self.logger.warning("Owner or repo not extracted from GitHub URL")
                    return "Unknown"
                ...
                self.logger.debug(f"Fetching version from: {raw_url}")
                ...
                try:
                    ...
                    with urllib.request.urlopen(request, context=context) as response:
                        ...
                        if match:
                            ...
                            self.logger.warning("Could not find version in pyproject.toml content")
                            return "Unknown"
                            
                except urllib.error.HTTPError as e:
                    self.logger.error(f"HTTP Error {e.code} when accessing {raw_url}")
                    if e.code == 404:
                        self.logger.error(f"Could not find pyproject.toml at {raw_url}")
                        self.logger.error(f"Check that the repository exists and branch '{self.branch}' is correct")
                    return "Unknown"
                except urllib.error.URLError as e:
                    self.logger.error(f"URL Error when accessing GitHub: {e.reason}")
                    return "Unknown"
                    
            except Exception as e:
                self.logger.error(f"Unexpected error getting GitHub version: {type(e).__name__}: {str(e)}")
                return "Unknown"
