...
class RepoUpdater:
    ...
    def __init__(
        ...
        if match:
            ...
            self.logger.info(
                f"Extracted GitHub info - Owner: George Symonds, Repo: {self.repo}"
            )
        else:
            self.owner = None
            self.repo = None
    ...
    def get_local_version(self) -> str:
        """
        Get the local version from config.json.
        ...
        try:
            ...
            if getattr(sys, "frozen", False):
                ...
                config_paths = [os.path.join(sys._MEIPASS, "config.json")]
            else:
                # In development
                config_paths = [
                ...
                if os.path.exists(config_path):
                    with open(config_path, "r") as f:
                        ...
                        if "version" in config:
                            self.logger.info(
                                f"Got version from config.json: {config['version']}"
                            )
                            return config["version"]
        except Exception as e:
            self.logger.warning(f"Error reading version from config.json: {e}")

        return "2.1.0"  # Default fallback version matching current
    ...
    def is_version_too_old(
        self,
        local_version: str,
        github_version: str,
        max_major_behind: int = 0,
        max_minor_behind: int = 1,
    ) -> bool:
        """
        ...
        try:
            ...
            if major_diff > max_major_behind:
                ...
                self.logger.warning(
                    f"Version too old: {major_diff} major versions behind (max allowed: {max_major_behind})"
                )
                return True
            ...
            if major_diff == 0:
                ...
                if minor_diff > max_minor_behind:
                    ...
                    self.logger.warning(
                        f"Version too old: {minor_diff} minor versions behind (max allowed: {max_minor_behind})"
                    )
                    return True
    ...
    def check_and_update(
        self, parent_window=None, block_if_too_old: bool = True
    ) -> dict:
        """
        ...
        if version_check["error"]:
            ...
            if self.dialog_helper and parent_window:
                self.dialog_helper.show_message(
                    ...
                    self.dialog_helper.t("Could not check for updates")
                    + f": {version_check['error']}",
                    message_type="error",
                )

            return {
                "success": False,
                "message": version_check["error"],
                "blocked": False,
            }
        ...
        if block_if_too_old and version_check["github_version"] != "Unknown":
            is_too_old = self.is_version_too_old(
                version_check["local_version"], version_check["github_version"]
            )

            if is_too_old:
                self.logger.error(
                    f"Version {version_check['local_version']} is too old compared to {version_check['github_version']}"
                )
        ...
        if not version_check["update_available"]:
            ...
            if self.dialog_helper and parent_window:
                self.dialog_helper.show_message(
                    ...
                    self.dialog_helper.t("You are running the latest version.")
    ...
    def _add_logo_header(self, parent_frame, theme_colors):
        ...
        header_container = tk.Frame(
            parent_frame, bg=theme_colors["background"] if theme_colors else "white"
        )
    ...
    def show_version_dialog(self, parent_window, version_check, *, is_blocking=False):
        ...
        dialog = self.dialog_helper.create_dialog(
            parent_window, self.dialog_helper.t(title), modal=True, topmost=True
        )
        ...
        theme_colors = None
        if (
            hasattr(self.dialog_helper, "gui_manager")
            and self.dialog_helper.gui_manager
        ):
        ...
        if is_blocking:
            version_text = (
                self.dialog_helper.t(
                    "This version of GeoVue is too old and cannot be used."
                )
                + "\n\n"
                + self.dialog_helper.t("Your version")
                + f": {version_check['local_version']}\n"
                + self.dialog_helper.t("Latest version")
                + f": {version_check['github_version']}\n\n"
            ...
            version_text = (
                ...
                + "\n\n"
                + self.dialog_helper.t("Current version")
                + f": {version_check['local_version']}\n"
                + self.dialog_helper.t("Latest version")
                + f": {version_check['github_version']}"
            )
        ...
        def on_enter(e):
            email_label.config(
                fg=theme_colors["accent_green"] if theme_colors else "#00aa00"
            )
