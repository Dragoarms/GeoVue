"""
3/4-Class Cell Classifier for GeoVue Chip Trays
...
class CellLabels:
    ...
    @classmethod
    def labels_for_num_classes(cls, num_classes: int) -> List[str]:
        if num_classes < 1 or num_classes > len(cls.ORDER):
            raise ValueError(f"num_classes must be between 1 and {len(cls.ORDER)}")
        return list(cls.ORDER[:num_classes])
...
class CellSample:
    ...
    hole_id: str
    depth_code: str
    source: str  # "folder", "filename", or "register"


class CellDataset:
    """Dataset manager for cell classification."""

    def __init__(self, config: ClassifierConfig):
        self.config = config
        self.label_names = CellLabels.labels_for_num_classes(config.num_classes)
    ...
    def _folder_label(self, scan_dir: Path, img_file: Path) -> Optional[str]:
        """Return a class label from the nearest label folder, if present."""
        label_lookup = {label.lower(): label for label in CellLabels.ORDER}
        try:
            rel_parts = img_file.relative_to(scan_dir).parts[:-1]
        except ValueError:
            rel_parts = img_file.parts[:-1]
        ...
        return label_lookup.get(scan_dir.name.lower())
    ...
    def scan_images(self) -> int:
        ...
        Labeling logic:
        1. If image is under a label folder (Dry/Wet/Empty/Bad), use that label.
        2. If filename ends with _Empty.png -> Empty.
        3. If filename ends with _Bad.png -> Bad.
        4. If image key in bad_images set (from register) -> Bad.
        5. If filename ends with _Wet.png -> Wet.
        6. If filename ends with _Dry.png -> Dry.
        7. Otherwise -> unlabeled (skipped).
        """
        self.samples = []
        ...
        # Pattern: {HoleID}_CC_{DepthCode}[_temp][_Label].png
        # Handles folder-labelled datasets where filename suffixes may be stale.
        pattern = re.compile(
            r"^([A-Za-z0-9]+)_CC_(\d{3,4})(?:_temp)?(?:_(Wet|Dry|Empty|Bad))?\.png$",
            re.IGNORECASE
        )
        ...
        for scan_dir in scan_dirs:
            ...
            for img_file in scan_dir.rglob("*.png"):
                ...
                filename_label = match.group(3).capitalize() if match.group(3) else None

                # Check if this image is marked Bad in register
                image_key = f"{hole_id}_CC_{depth_code}"

                # Determine final label. Folder label is treated as ground truth.
                folder_label = self._folder_label(scan_dir, img_file)
                if folder_label:
                    label = folder_label
                    source = "folder"
                elif filename_label == "Bad":
        ...
        inactive_labels = [label for label in CellLabels.ORDER if label not in self.label_names]
        for label in inactive_labels:
            if self.stats[label]:
                print(f"  - {label}: {self.stats[label]} (skipped; inactive)")
        print(f"  - Unlabeled (skipped): {self.stats['unlabeled']}")
        if self.stats["skipped_inactive"]:
            print(f"  - Inactive labels skipped: {self.stats['skipped_inactive']}")

        return len(self.samples)

    def get_class_weights(self) -> List[float]:
        """Calculate class weights for imbalanced data."""
        counts = [max(self.stats[label], 1) for label in self.label_names]
        total = sum(counts)
