...
class CompartmentRegistrationDialog:
    ...
    def __init__(self, parent, image, detected_boundaries, missing_marker_ids=None,
                ...
                show_adjustment_controls=True, image_path=None):
        """
        ...
        # Get compartment interval from metadata (default to 1.0 if not specified)
    ...
    def _check_missing_corner_markers(self):
            ...
            # Log what we found
            self.logger.debug(f"Corner marker detection - 0: {has_marker_0}, 1: {has_marker_1}, 2: {has_marker_2}, 3: {has_marker_3}")
            
            # Convert missing_marker_ids to a list if it isn't already
            if not isinstance(self.missing_marker_ids, list):
                self.missing_marker_ids = list(self.missing_marker_ids) if self.missing_marker_ids else []
            ...
            if has_marker_0 or has_marker_1:
                ...
                if 0 in self.missing_marker_ids:
                    ...
                    self.logger.debug("Removed marker 0 from missing list - already have a top corner")
                if 1 in self.missing_marker_ids:
                    self.missing_marker_ids.remove(1)
                    self.logger.debug("Removed marker 1 from missing list - already have a top corner")
            else:
            ...
            if has_marker_2 or has_marker_3:
                ...
                if 2 in self.missing_marker_ids:
                    ...
                    self.logger.debug("Removed marker 2 from missing list - already have a bottom corner")
                if 3 in self.missing_marker_ids:
                    self.missing_marker_ids.remove(3)
                    self.logger.debug("Removed marker 3 from missing list - already have a bottom corner")
            else:
            ...
            self.logger.debug(f"Updated missing marker IDs after corner check: {self.missing_marker_ids}")
    ...
    def _calculate_vertical_constraints(self):
        ...
        for marker_id in [0, 1]:  # Top corners
            ...
            if marker_id in self.markers:
                ...
                self.logger.debug(f"Found top corner {marker_id} in markers, using top edge at y={top_edge_y}")
            
            # Check in self.corner_markers - use TOP edge for detected markers
            if hasattr(self, 'corner_markers') and self.corner_markers and marker_id in self.corner_markers:
                ...
                if marker_id not in self.result_boundaries:  # Not manually placed
                    ...
                    if top_edge_y not in top_y_coords:
                        top_y_coords.append(top_edge_y)
                        self.logger.debug(f"Found top corner {marker_id} in corner_markers, using top edge at y={top_edge_y}")
                else:
                    # For manually placed, use center
                    center_y = np.mean(corners[:, 1])
                    if center_y not in top_y_coords:
                        top_y_coords.append(center_y)
                        self.logger.debug(f"Found top corner {marker_id} in corner_markers (manual), using center at y={center_y}")
            
            # Check in self.result_boundaries (manually placed) - use center
            if hasattr(self, 'result_boundaries') and marker_id in self.result_boundaries:
                corners = self.result_boundaries[marker_id]
                if isinstance(corners, np.ndarray):
                    center_y = np.mean(corners[:, 1])
                    if center_y not in top_y_coords:  # Avoid duplicates
                        top_y_coords.append(center_y)
                        self.logger.debug(f"Found top corner {marker_id} in result_boundaries (manual) at y={center_y}")
        ...
        for marker_id in [2, 3]:  # Bottom corners
            ...
            if marker_id in self.markers:
                ...
                self.logger.debug(f"Found bottom corner {marker_id} in markers, using bottom edge at y={bottom_edge_y}")
            ...
            if hasattr(self, 'corner_markers') and self.corner_markers and marker_id in self.corner_markers:
                ...
                if marker_id not in self.result_boundaries:  # Not manually placed
                    ...
                    if bottom_edge_y not in bottom_y_coords:
                        bottom_y_coords.append(bottom_edge_y)
                        self.logger.debug(f"Found bottom corner {marker_id} in corner_markers, using bottom edge at y={bottom_edge_y}")
                else:
                    # For manually placed, use center
                    center_y = np.mean(corners[:, 1])
                    if center_y not in bottom_y_coords:
                        bottom_y_coords.append(center_y)
                        self.logger.debug(f"Found bottom corner {marker_id} in corner_markers (manual), using center at y={center_y}")
            ...
            if hasattr(self, 'result_boundaries') and marker_id in self.result_boundaries:
                ...
                if isinstance(corners, np.ndarray):
                    ...
                    if center_y not in bottom_y_coords:  # Avoid duplicates
                        bottom_y_coords.append(center_y)
                        self.logger.debug(f"Found bottom corner {marker_id} in result_boundaries (manual) at y={center_y}")
    ...
    def _create_dialog(self):
        ...
        try:
            ...
            dialog = DialogHelper.create_dialog(
                ...
                modal=True,
                topmost=True,
                size_ratio=0.9,  # Larger size ratio for this dialog
                min_width=1000,  # Increased minimum width
                min_height=700   # Increased minimum height
            )
    ...
    def _create_widgets(self):
        ...
        if self.gui_manager:
            ...
            style.configure('Content.TFrame', background=self.theme_colors["background"])
            style.configure('Content.TLabel', background=self.theme_colors["background"], 
        ...
        if self.boundaries_viz is not None:
            img_h, img_w = self.boundaries_viz.shape[:2]
            # Calculate initial scale to fit in reasonable window size
            max_width = 800
            max_height = 600
            scale_w = max_width / img_w
            scale_h = max_height / img_h
            initial_scale = min(scale_w, scale_h, 1.0)  # Don't scale up beyond 1.0
            
            initial_width = int(img_w * initial_scale)
            initial_height = int(img_h * initial_scale)
            
            # Configure canvas with initial size
        ...
        self.metadata_frame = ttk.Frame(self.bottom_container, style='Content.TFrame', padding=10)
        ...
        if self.current_mode == self.MODE_ADJUST_BOUNDARIES and self.adjustment_controls_visible:
            self.adjustment_frame.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10))
        ...
        def create_mode_button(text, mode, position, container):
            if self.gui_manager:
                ...
                button = self.gui_manager.create_modern_button(
                    ...
                    color=self.theme_colors["accent_green"] if self.current_mode == mode else self.theme_colors["field_bg"],
    ...
    def _create_metadata_panel(self, parent_frame):
        ...
        if self.gui_manager:
            ...
            self.increment_button.grid(row=row, column=0, columnspan=3, pady=(10, 5), sticky=tk.W+tk.E)
        else:
    ...
    def _init_zoom_lens(self):
        ...
        try:
            ...
            if not hasattr(self, 'dialog') or not self.dialog or not self.dialog.winfo_exists():
                self.logger.warning("Cannot create zoom lens - dialog doesn't exist or isn't visible")
                return
            ...
            self._zoom_canvas.create_line(0, center_y, zoom_width, center_y, fill='red', width=1, tags='crosshair')
            self._zoom_canvas.create_line(center_x, 0, center_x, zoom_height, fill='red', width=1, tags='crosshair')
            self._zoom_canvas.create_oval(center_x-3, center_y-3, center_x+3, center_y+3, fill='red', outline='red', tags='crosshair')
            
            # Add crosshair to flipped lens
            self._zoom_canvas_flipped.create_line(0, center_y, zoom_width, center_y, fill='red', width=1, tags='crosshair')
            self._zoom_canvas_flipped.create_line(center_x, 0, center_x, zoom_height, fill='red', width=1, tags='crosshair')
            self._zoom_canvas_flipped.create_oval(center_x-3, center_y-3, center_x+3, center_y+3, fill='red', outline='red', tags='crosshair')
    ...
    def _update_interface_for_mode(self):
        ...
        if self.current_mode == self.MODE_METADATA:
            ...
            if self.adjustment_controls_visible and not self.adjustment_frame.winfo_ismapped():
                self.adjustment_frame.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10))
    ...
    def _update_static_zoom_views(self):
                ...
                try:
                    ...
                    cv2.line(left_np, (0, new_height//2), (new_width, new_height//2), (0, 255, 0), 1)
                    cv2.line(left_np, (new_width//2, 0), (new_width//2, new_height), (0, 255, 0), 1)
                    
                    cv2.line(right_np, (0, new_height//2), (new_width, new_height//2), (0, 255, 0), 1)
                    cv2.line(right_np, (new_width//2, 0), (new_width//2, new_height), (0, 255, 0), 1)
                        ...
                        for marker_id in [0, 3]:
                            if marker_id in self.corner_markers:
                                ...
                                if 0 <= zoom_x < new_width and 0 <= zoom_y < new_height:
                                    ...
                                    cv2.putText(left_np, str(marker_id), (zoom_x-8, zoom_y-8),
                        ...
                        for marker_id in [1, 2]:
                            if marker_id in self.corner_markers:
                                ...
                                if 0 <= zoom_x < new_width and 0 <= zoom_y < new_height:
                                    ...
                                    # Smaller font (0.3 instead of 0.5)
                                    cv2.putText(right_np, str(marker_id), (zoom_x-8, zoom_y-8),
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 0, 0), 1)
                    ...
                    left_canvas = Image.new('RGB', (self.zoom_width, self.zoom_height), 
                                          color=bg_color)
                    right_canvas = Image.new('RGB', (self.zoom_width, self.zoom_height), 
                                           color=bg_color)
                    ...
                    self.left_zoom_canvas.create_image(self.zoom_width//2, self.zoom_height//2, image=self.left_zoom_photo)
                    self.right_zoom_canvas.create_image(self.zoom_width//2, self.zoom_height//2, image=self.right_zoom_photo)
                    
                    # Add titles back with smaller font
                    self.left_zoom_canvas.create_text(
                        self.zoom_width//2, 15, 
                    ...
                    right_zoom_x = max(10, min(screen_width - self.zoom_width - 10, right_zoom_x))
                    zoom_y = max(10, min(screen_height - self.zoom_height - 10, zoom_y))
                    
                    # Set positions and show the windows
                    self.left_zoom_window.geometry(f"{self.zoom_width}x{self.zoom_height}+{left_zoom_x}+{zoom_y}")
                    self.right_zoom_window.geometry(f"{self.zoom_width}x{self.zoom_height}+{right_zoom_x}+{zoom_y}")                    
                    # Show windows
    ...
    def _update_compartment_labels(self, *args):
        ...
        try:
            ...
            if depth_from_str and depth_to_str and depth_from_str.isdigit() and depth_to_str.isdigit():
                # Parse depth values
                depth_from = int(depth_from_str)
                ...
                if compartment_count > 0:
                    ...
                    # Update visualization to show new labels
                    self._update_visualization()
        ...
        try:
            ...
            if not hole_id:
                DialogHelper.show_message(self.dialog, "Validation Error", 
                                        "Hole ID is required", message_type="error")
                return False
            ...
            if not depth_from_str or not depth_to_str:
                DialogHelper.show_message(self.dialog, "Validation Error", 
                                        "Depth From and Depth To are required", message_type="error")
                return False
            ...
            try:
                ...
                if depth_from < 0 or depth_to < 0:
                    DialogHelper.show_message(self.dialog, "Validation Error", 
                                            "Depth values cannot be negative", message_type="error")
                    return False
                ...
                DialogHelper.show_message(self.dialog, "Validation Error", 
                                        "Depth values must be whole numbers", message_type="error")
                return False
            ...
            DialogHelper.show_message(self.dialog, "Error", 
                                    f"An error occurred during validation: {str(e)}", message_type="error")
            return False
    ...
    def _on_increment_from_last(self):
            ...
            try:
                ...
                if not self.app or not hasattr(self.app, 'get_incremented_metadata'):
                    DialogHelper.show_message(self.dialog, "Not Available", 
                                            "Increment from last is not available", message_type="warning")
                    return
                ...
                if incremented and incremented.get('hole_id'):
                    ...
                    self.status_var.set(f"Auto-filled with incremented metadata from last core")
                else:
    ...
    def _get_instruction_text(self):
        ...
        if self.current_mode == self.MODE_METADATA:
            ...
            if self.missing_marker_ids and not self.annotation_complete:
                current_id = self.missing_marker_ids[self.current_index] if self.current_index < len(self.missing_marker_ids) else None
                if current_id == 24:
                    return DialogHelper.t("Place Metadata Marker (ID 24)")
                elif current_id in [0, 1, 2, 3]:  # Corner markers
                    if current_id in [0, 1]:
                        return DialogHelper.t("Click to set the TOP boundary constraint")
                    else:
                        return DialogHelper.t("Click to set the BOTTOM boundary constraint")
                else:
                    # Use marker_to_compartment mapping for depth display
                    compartment_number = self.marker_to_compartment.get(current_id, current_id - 3)
                    return DialogHelper.t(f"Place Compartment at Depth {compartment_number}m")
            elif self.annotation_complete:
                return DialogHelper.t("All Markers Placed - Click Continue to Adjustment")
            else:
                return DialogHelper.t("No Missing Markers - Click Continue to Adjustment")
    ...
    def _update_status_message(self):
        ...
        if self.current_mode == self.MODE_METADATA:
            ...
            if self.missing_marker_ids and not self.annotation_complete:
                current_id = self.missing_marker_ids[self.current_index] if self.current_index < len(self.missing_marker_ids) else None
                if current_id == 24:
                    self.status_var.set(DialogHelper.t("Click to place the metadata marker"))
                elif current_id in [0, 1, 2, 3]:  # Corner markers
                    if current_id in [0, 1]:
                        self.status_var.set(DialogHelper.t(
    ...
    def _adjust_height(self, delta: int) -> None:
        ...
        self.logger.debug(f"After adjustment - top_y: {self.top_y}, bottom_y: {self.bottom_y}")
    ...
    def _adjust_side_height(self, side, delta):
        ...
        self.logger.debug(f"Before side adjustment - left_offset: {self.left_height_offset}, right_offset: {self.right_height_offset}")
        ...
        self.logger.debug(f"After side adjustment - left_offset: {self.left_height_offset}, right_offset: {self.right_height_offset}")
    ...
    def _apply_adjustments(self):
        ...
        try:
            self.logger.debug(f"Applying adjustments - top_y: {self.top_y}, bottom_y: {self.bottom_y}, " 
        ...
        try:
            ...
            if self.current_mode == self.MODE_METADATA:
                ...
                for existing_id, existing_corners in self.markers.items():
                    ...
                    if cv2.pointPolygonTest(existing_corners.astype(np.int32), (image_x, image_y), False) >= 0:
                        self.status_var.set(DialogHelper.t(f"Cannot place marker here - overlaps with marker {existing_id}"))
                        return
                ...
                for placed_id, placed_boundary in self.result_boundaries.items():
                    if placed_id == 24:  # Metadata marker
                        ...
                        if np.linalg.norm(click_point - center) < 30:  # 30 pixel minimum distance
                            self.status_var.set(DialogHelper.t(f"Cannot place marker here - too close to marker {placed_id}"))
                            return
                    # FIXED: Check if corner marker (stored as numpy array)
                        ...
                        if isinstance(placed_boundary, np.ndarray):
                            ...
                            center = np.mean(placed_boundary, axis=0)
                            if np.linalg.norm(click_point - center) < 30:  # 30 pixel minimum distance
                                self.status_var.set(DialogHelper.t(f"Cannot place marker here - too close to corner marker {placed_id}"))
                                return
                            
                    else:  # Compartment marker
                        # Ensure it's a tuple before unpacking
                        if isinstance(placed_boundary, tuple) and len(placed_boundary) == 4:
                            x1, y1, x2, y2 = placed_boundary
                            # Check if click is within compartment bounds
                            if x1 <= image_x <= x2 and y1 <= image_y <= y2:
                                self.status_var.set(DialogHelper.t(f"Cannot place marker here - overlaps with compartment {placed_id}"))
                                return
                        elif isinstance(placed_boundary, np.ndarray):
                            # Handle case where compartment boundary might be stored as array
                            self.logger.warning(f"Unexpected array format for compartment {placed_id}")
                ...
                if current_id not in [24, 0, 1, 2, 3]:
                    if self._would_overlap_existing(image_x):
                        self.status_var.set(DialogHelper.t("Cannot place compartment here - overlaps with existing compartment"))
                        return                
                ...
                if current_id == 24:  # Metadata marker
                    ...
                    # Display feedback
                    self.status_var.set(DialogHelper.t(f"Placed metadata marker at x={image_x}, y={image_y}"))
                    ...
                    corners = np.array([
                        ...
                        [marker_x + half_size, marker_y + half_size],  # Bottom-right
                    ...
                    if not hasattr(self, 'corner_markers') or self.corner_markers is None:
                        self.corner_markers = {}
                    ...
                    # Display feedback
                    if current_id in [0, 1]:
                        self.status_var.set(DialogHelper.t(f"Set TOP constraint at y={image_y}"))
                    else:
                        self.status_var.set(DialogHelper.t(f"Set BOTTOM constraint at y={image_y}"))

                    # ===================================================
                    # Recalculate vertical constraints after placing corner marker
                    # ===================================================
                    self._calculate_vertical_constraints()
                    self.logger.debug(f"Updated vertical constraints after placing corner marker: top_y={self.top_y}, bottom_y={self.bottom_y}")
                    ...
                    if current_id in [0, 1]:  # Placed a top corner
                        ...
                        if other_top in self.missing_marker_ids:
                            ...
                            self.logger.debug(f"Removed marker {other_top} from missing list - already have a top corner")
                        ...
                        if other_bottom in self.missing_marker_ids:
                            ...
                            self.logger.debug(f"Removed marker {other_bottom} from missing list - already have a bottom corner")
                    ...
                    # Calculate slopes
                    top_y_slope = (right_top_y - left_top_y) / img_width if img_width > 0 else 0
                    y1 = int(left_top_y + (top_y_slope * x1))
                    
                    x2 = min(img_width - 1, image_x + half_width)
                    bottom_y_slope = (right_bottom_y - left_bottom_y) / img_width if img_width > 0 else 0
                    ...
                    depth = self.marker_to_compartment.get(current_id, current_id - 3)
                    self.status_var.set(DialogHelper.t(f"Placed compartment at depth {depth}m (x={image_x})"))
    ...
    def _render_zoom(self, canvas, event, flipped=False):
        ...
        zoom_width, zoom_height = getattr(self, '_zoom_width', 250), getattr(self, '_zoom_height', 350)
        ...
        # ===================================================
        if self.current_mode == self.MODE_MISSING_BOUNDARIES and not self.annotation_complete:
            # For missing boundaries mode, show the compartment placement area
            # Extract region around where compartment will be placed
            zoom_radius_x = zoom_width // 4  # Extract a quarter of the zoom window width
            zoom_radius_y = zoom_height // 4  # Extract a quarter of the zoom window height
            ...
            if self.current_index < len(self.missing_marker_ids):
                ...
                if current_id != 24:  # Only for compartments, not metadata marker
                    ...
                    preview_color = (0, 0, 255) if would_overlap else (255, 0, 255)  # Red if would overlap
                    cv2.rectangle(region, (x1, y1), (x2, y2), preview_color, 2)  # Thinner line for zoom
                    
                    # Add current compartment number
                    display_number = self.marker_to_compartment.get(current_id, current_id - 3)
                    
                    mid_x = (x1 + x2) // 2
                    mid_y = (y1 + y2) // 2
                    
                    # Smaller text for zoom view
                    text_size = cv2.getTextSize(f"{display_number}m", cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2)[0]
                    cv2.rectangle(region, 
                                (mid_x - text_size[0]//2 - 3, mid_y - text_size[1]//2 - 3),
                                (mid_x + text_size[0]//2 + 3, mid_y + text_size[1]//2 + 3),
                                (0, 0, 0), -1)
                    
                    # Add text with 'm' suffix
                    cv2.putText(region, f"{display_number}m", 
                            (mid_x - text_size[0]//2, mid_y + text_size[1]//2), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, preview_color, 2)
        ...
        try:
            ...
            if flipped:
                ...
                self._zoom_canvas_flipped.create_line(
                    center_x, 0, center_x, zoom_height, fill='red', width=1, tags='crosshair'
                )
                ...
                self._zoom_lens_flipped.geometry(f"{zoom_width}x{zoom_height}+{global_x}+{global_y}")
                ...
                if self.current_mode == self.MODE_MISSING_BOUNDARIES:
                    ...
                    canvas_top_y = int(self.top_y * self.scale_ratio + self.canvas_offset_y)
                    lens_y = max(10, canvas.winfo_rooty() + canvas_top_y - zoom_height - 30)
                    
                    # Center horizontally on cursor
                    screen_width = self.dialog.winfo_screenwidth()
                    lens_x = min(max(10, canvas.winfo_rootx() + event.x - zoom_width // 2), 
                ...
                self._zoom_lens.geometry(f"{zoom_width}x{zoom_height}+{lens_x}+{lens_y}")
        ...
        try:
            ...
            if self.markers:
                ...
                for marker_id, corners in self.markers.items():
                    ...
                    # Always draw marker outline
                    cv2.polylines(viz_image, [corners_int.reshape(-1, 1, 2)], True, (0, 0, 255), 2)
                    ...
                    # Only draw text labels if not in fast mode
                    if not fast_mode:
                        # Add background for marker ID text
                        text_size = cv2.getTextSize(f"{marker_id}", cv2.FONT_HERSHEY_SIMPLEX, 0.8, 2)[0]
                        cv2.rectangle(viz_image, 
                                    (center_x - text_size[0]//2 - 5, center_y - text_size[1]//2 - 15),
                                    (center_x + text_size[0]//2 + 5, center_y + text_size[1]//2 - 5),
                                    (0, 0, 0), -1)
                        
                        # Draw marker ID
                        cv2.putText(viz_image, f"{marker_id}", 
            ...
            if self.current_mode == self.MODE_METADATA:
                ...
                if self.annotation_complete:
                    ...
                    if self.result_boundaries and self.current_index > 0:
                        ...
                        # Get depth for display using marker_to_compartment mapping
                        depth = self.marker_to_compartment.get(current_id, current_id - 3)
                        self.status_var.set(DialogHelper.t(f"Undid compartment at depth {depth}m. Please place it again."))
                else:
        ...
        try:
            ...
            if self.current_mode == self.MODE_METADATA:
                ...
                if not self.missing_marker_ids or self.annotation_complete:
                    ...
                    if missing_count > 0:
                        if DialogHelper.confirm_dialog(
                            ...
                            DialogHelper.t(f"You have {missing_count} compartments left to annotate. "
                ...
                self.final_results = {
                    ...
                    'final_visualization': self.current_viz if hasattr(self, 'current_viz') else None,
    ...
    def _show_dialog_after_delay(self):
        """
        Helper method to show dialog after delay.
        This allows the window to be properly rendered before interacting with it.
        """
        self.logger.debug("_show_dialog_after_delay called")
        try:
            ...
            # Set grab after delay
            self.dialog.grab_set()
            self.logger.debug("Dialog should now be fully visible and interactive")
        except Exception as e:
            self.logger.error(f"Error in _show_dialog_after_delay: {str(e)}")
    ...
    def show(self):
        ...
        if current_thread is not threading.main_thread():
            self.logger.error(f"show() called from non-main thread: {current_thread.name}")
            return {}
        
        try:
            # Make sure dialog exists
            self.logger.debug("Checking if dialog exists")
            if not hasattr(self, 'dialog') or not self.dialog:
                self.logger.error("Cannot show dialog - dialog does not exist")
                return {}
            
            self.logger.debug("Dialog exists, proceeding to center on parent")
            # Center dialog on parent
            if self.parent:
                self.logger.debug("Parent exists, setting transient")
                self.dialog.transient(self.parent)
                
                # Center on parent - be defensive about parent size
                try:
                    self.logger.debug("Getting parent and dialog dimensions")
                    parent_width = self.parent.winfo_width() or 800
                    parent_height = self.parent.winfo_height() or 600
                    
                    # Now we can get more accurate dialog dimensions since it's visible
                    dialog_width = self.dialog.winfo_width() or 1000
                    dialog_height = self.dialog.winfo_height() or 700
                    
                    self.logger.debug(f"Parent dimensions: {parent_width}x{parent_height}")
                    self.logger.debug(f"Dialog dimensions: {dialog_width}x{dialog_height}")
                    
                    x = self.parent.winfo_rootx() + max(0, (parent_width - dialog_width) // 2)
                    y = self.parent.winfo_rooty() + max(0, (parent_height - dialog_height) // 2)
                    
                    self.logger.debug(f"Calculated position: x={x}, y={y}")
                    
                    # Ensure dialog is visible on screen
                    x = max(0, x)
                    y = max(0, y)
                    
                    self.logger.debug(f"Final position: x={x}, y={y}")
                    self.dialog.geometry(f"+{x}+{y}")
                except Exception as e:
                    self.logger.warning(f"Error centering dialog: {str(e)}")
            
            # Ensure the dialog is visible and ready
            self.logger.debug("Ensuring dialog is visible")
            self.dialog.lift()
            self.dialog.state('normal')  # Ensure not withdrawn
            self.dialog.deiconify()
            self.dialog.update()  # Process any pending events
            
            # Add a short delay to ensure UI is properly initialized
            self.logger.debug("Adding delay before showing dialog")
            self.dialog.after(200, self._show_dialog_after_delay)

            # Verify dialog exists before waiting
            self.logger.debug("About to call wait_window")
            if hasattr(self, 'dialog') and self.dialog and self.dialog.winfo_exists():
                try:
                    is_visible = self.dialog.winfo_ismapped()
                    self.logger.debug(f"Dialog is visible: {is_visible}")
                    
                    if not is_visible:
                        self.logger.debug("Dialog not visible, forcing visibility")
                        self.dialog.deiconify()
                        self.dialog.update()
                    ...
                    self.logger.error(f"Error in wait_window operation: {str(e)}")
                    self.logger.error(traceback.format_exc())
                    return {}
            else:
                self.logger.debug("Dialog no longer exists, cannot wait_window")
                return {}  # Return empty result if dialog doesn't exist

            self.logger.debug(f"Returning final results: {hasattr(self, 'final_results')}")
            
            # Return the final results
            if hasattr(self, 'final_results'):
                return self.final_results
            else:
                # Create a default result dict with both metadata and boundaries
                result = {
                    'result_boundaries': self.result_boundaries,
                    'top_boundary': self.top_y,
                    'bottom_boundary': self.bottom_y,
                    ...
                    # Include metadata
                    'hole_id': self.hole_id.get().strip(),
                    'depth_from': int(self.depth_from.get().strip()) if self.depth_from.get().strip().isdigit() else None,
                    'depth_to': int(self.depth_to.get().strip()) if self.depth_to.get().strip().isdigit() else None,
                    'compartment_interval': self.interval_var.get()
                }
                return result
