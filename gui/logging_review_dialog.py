...
class LoggingReviewDialog:
    ...
    def __init__(self, parent, file_manager, gui_manager, onedrive_manager, config, json_manager=None):
    ...
    def _find_compartment_image(self, hole_id: str, compartment_depth: int) -> Optional[str]:
        ...
        project_folder = os.path.join(approved_path, project_code)
        hole_path = os.path.join(project_folder, hole_id)
            ...
            for filename in os.listdir(hole_path):
                if filename.startswith(expected_prefix):
                    ...
                    if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.tiff', '.tif')):
                        return os.path.join(hole_path, filename)
