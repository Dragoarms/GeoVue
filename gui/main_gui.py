...
class MainGUI:
    ...
    def __init__(self, app):
        ...
        # Initialize OneDrive path manager with file_manager and config for consistent paths
        self.onedrive_manager = OneDrivePathManager(
            self.root, 
            self.file_manager, 
        ...
        # set up the onedrive paths if available
        self.setup_onedrive_paths()
        
        # Initialize visualization cache for dialogs
        self.visualization_cache = {}
        
        # Track QAQC manager - will be initialized when needed
        self.qaqc_manager = None
    ...
    def create_gui(self):
        ...
        # Local output
        self.output_folder_var = tk.StringVar(value=self.file_manager.dir_structure['processed_originals'])
        ...
        onedrive_manager = OneDrivePathManager(self.root, silent=True)
        ...
        # Set display values using OneDrivePathManager's paths or config
        config = self.app.config_manager.as_dict()
        
        # Approved folder
        approved_path = onedrive_manager.get_approved_folder_path() or config.get('onedrive_approved_folder', "")
        self.approved_path_var.set(approved_path)
        
        # Processed originals
        processed_path = onedrive_manager.get_processed_originals_path() or config.get('onedrive_processed_originals', "")
        self.processed_originals_path_var.set(processed_path)
        
        # Drill traces
        drill_traces_path = onedrive_manager.get_drill_traces_path() or config.get('onedrive_drill_traces', "")
        self.drill_traces_path_var.set(drill_traces_path)
        
        # Register
        register_path = onedrive_manager.get_register_path() or config.get('onedrive_register_path', "")
        self.register_path_var.set(register_path)
        ...
        for lang_code in languages:
            ...
            language_items.append({
                ...
                'command': lambda lc=lang_code: self.change_language(lc)  # Use lambda with default arg to capture value
            })

        menu_defs[self.t("Language")] = language_items

        print(f"Languages available: {languages}")
        print(f"Language items created: {language_items}")
        ...
        try:
            ...
            if not register_path:
                DialogHelper.show_message(
                    ...
                    DialogHelper.t("No register path configured. Please check OneDrive settings."),
                    message_type="error"
                )
                return
            
            # Initialize JSON manager if needed
            base_path = os.path.dirname(register_path)
            if not JSONRegisterManager.has_existing_data_static(base_path):
                DialogHelper.show_message(
                    self.root,
                    DialogHelper.t("Error"),
                    DialogHelper.t("No register data found at the configured location."),
                    message_type="error"
                )
                return
            ...
            progress_dialog = ProgressDialog(
                self.root,
                DialogHelper.t("Validating Register"),
                DialogHelper.t("Initializing validation...")
            )
            ...
            try:
                ...
                if results and results.get('success', False):
                    message_lines = [
                        DialogHelper.t("Validation Complete"),
                        "",
                        DialogHelper.t("Files checked and register updated:"),
                        DialogHelper.t("Missing compartment images:") + f" {results.get('missing_files_compartments', 0)}",
                        DialogHelper.t("Missing original images:") + f" {results.get('missing_files_originals', 0)}"
                    ]
                    
                    # Add sync results
                    if results.get('compartments_added', 0) > 0:
                        message_lines.append(DialogHelper.t("New compartments found:") + f" {results['compartments_added']}")
                    if results.get('originals_added', 0) > 0:
                        message_lines.append(DialogHelper.t("New originals found:") + f" {results['originals_added']}")
                    if results.get('originals_updated', 0) > 0:
                        message_lines.append(DialogHelper.t("Originals updated:") + f" {results['originals_updated']}")
                    
                    # Add hex color results
                    if results.get('hex_colors_calculated', 0) > 0 or results.get('hex_colors_failed', 0) > 0:
                        message_lines.extend([
                            "",
                            DialogHelper.t("Hex colors calculated:") + f" {results.get('hex_colors_calculated', 0)}",
                            DialogHelper.t("Hex color failures:") + f" {results.get('hex_colors_failed', 0)}"
                        ])
                    ...
                    if total_missing > 0:
                        message_lines.extend([
                            "",
                            DialogHelper.t("Entries have been marked as MISSING_FILE in the register."),
                            DialogHelper.t("These files may have been deleted from OneDrive.")
                        ])
                        
                    DialogHelper.show_message(
                        self.root,
                        DialogHelper.t("Validation Results"),
                        "\n".join(message_lines),
                        message_type="info"
                    )
                else:
                    ...
                    DialogHelper.show_message(
                        self.root,
                        DialogHelper.t("Error"),
                        DialogHelper.t("Validation failed:") + f"\n{error_msg}",
                        message_type="error"
                    )
            ...
            DialogHelper.show_message(
                self.root,
                DialogHelper.t("Error"),
                DialogHelper.t("An error occurred:") + f"\n{str(e)}",
                message_type="error"
            )
    ...
    def setup_onedrive_paths(self):
        """
        Setup all OneDrive paths and save them to config.
        This should be called after OneDrivePathManager is initialized.
        """
        if not hasattr(self, 'onedrive_manager') or not self.onedrive_manager:
        ...
        try:
            ...
            if approved_path and os.path.exists(approved_path):
                self.app.config_manager.set('onedrive_approved_folder', approved_path)
                self.approved_path_var.set(approved_path)
                self.logger.info(f"Saved OneDrive approved folder: {approved_path}")
                paths_found = True
            
            # Processed originals
            processed_path = self.onedrive_manager.get_processed_originals_path()
            if processed_path and os.path.exists(processed_path):
                self.app.config_manager.set('onedrive_processed_originals', processed_path)
                self.processed_originals_path_var.set(processed_path)
                self.logger.info(f"Saved OneDrive processed originals: {processed_path}")
                paths_found = True
            
            # Rejected/Failed folder
            rejected_path = self.onedrive_manager.get_rejected_folder_path()
            if rejected_path and os.path.exists(rejected_path):
                self.app.config_manager.set('onedrive_rejected_folder', rejected_path)
                self.logger.info(f"Saved OneDrive rejected folder: {rejected_path}")
                paths_found = True
            
            # Drill traces
            drill_traces_path = self.onedrive_manager.get_drill_traces_path()
            if drill_traces_path and os.path.exists(drill_traces_path):
                self.app.config_manager.set('onedrive_drill_traces', drill_traces_path)
                self.drill_traces_path_var.set(drill_traces_path)
                self.logger.info(f"Saved OneDrive drill traces: {drill_traces_path}")
                paths_found = True
            
            # Excel register
            register_path = self.onedrive_manager.get_register_path()
            if register_path and os.path.exists(register_path):
                self.app.config_manager.set('onedrive_register_path', register_path)
                self.register_path_var.set(register_path)
                self.logger.info(f"Saved OneDrive register path: {register_path}")
                paths_found = True
            
            # Register Data folder
            register_data_path = self.onedrive_manager.get_register_data_path()
            if register_data_path and os.path.exists(register_data_path):
                self.app.config_manager.set('onedrive_register_data', register_data_path)
                self.logger.info(f"Saved OneDrive register data folder: {register_data_path}")
                paths_found = True
            ...
            self.logger.error(f"Error setting up OneDrive paths: {str(e)}")
            return False

    def _update_path_entry_colors(self):
        """Update the colors of path entry widgets based on validity."""
        # Find all entry widgets in the OneDrive collapsible section
        if hasattr(self, 'onedrive_collapsible'):
            for frame in self.onedrive_collapsible.content_frame.winfo_children():
    ...
    def _create_onedrive_path_field(self, parent, label_text, string_var, valid=False, is_file=False):
        ...
        browse_button = self.gui_manager.create_modern_button(
            ...
            text=self.t("Browse"), 
            color=self.gui_manager.theme_colors["accent_blue"],
            command=lambda: self._browse_onedrive_path(string_var, entry, is_file=is_file)
        )
        browse_button.pack(side=tk.RIGHT)
    ...
    def _update_gui_translations(self):
        ...
        try:
            ...
            if hasattr(self, 'onedrive_collapsible'):
                self.onedrive_collapsible.set_text(self.t("OneDrive Path Settings"))
            ...
            # Update OneDrive path labels
            onedrive_frame = self.onedrive_collapsible.content_frame
            for child in onedrive_frame.winfo_children():
    ...
    def _browse_onedrive_path(self, string_var, entry_widget=None, is_file=False):
        ...
        if is_file:
            ...
            if file_path:
                ...
                # ===================================================
                # Update OneDrive manager immediately
                # ===================================================
                self._update_single_onedrive_path(string_var.get(), is_file=True)
        else:
            # Browse for folder
            folder_path = filedialog.askdirectory(title=self.t("Select OneDrive folder"))
            if folder_path:
                ...
                # Update OneDrive manager immediately
                # ===================================================
                self._update_single_onedrive_path(string_var.get(), is_file=False)

    def _update_single_onedrive_path(self, path_value, is_file=False):
        """Update a single OneDrive path in the manager and save to config."""
        if not hasattr(self, 'onedrive_manager'):
            self.onedrive_manager = OneDrivePathManager(self.root)
        
        # Determine which path was updated and save to config
        # TODO - Add the 
        if is_file and path_value.endswith('.xlsx'):
            # Excel register
            self.onedrive_manager._register_path = path_value if path_value else None
            self.app.config_manager.set("onedrive_register_path", path_value)
        elif "Approved" in path_value:
            self.onedrive_manager._approved_folder_path = path_value if path_value else None
            self.app.config_manager.set("onedrive_approved_folder", path_value)
        elif "Processed Originals" in path_value or "4) Processed Originals" in path_value:
            self.onedrive_manager._processed_originals_path = path_value if path_value else None
            self.app.config_manager.set("onedrive_processed_originals", path_value)
        elif "Drill Traces" in path_value or "3) Drillhole Traces" in path_value:
            self.onedrive_manager._drill_traces_path = path_value if path_value else None
            self.app.config_manager.set("onedrive_drill_traces", path_value)
        else:
            # Try to determine based on current variable values
            if path_value == self.approved_path_var.get():
                self.onedrive_manager._approved_folder_path = path_value if path_value else None
                self.app.config_manager.set("onedrive_approved_folder", path_value)
            elif path_value == self.processed_originals_path_var.get():
                self.onedrive_manager._processed_originals_path = path_value if path_value else None
                self.app.config_manager.set("onedrive_processed_originals", path_value)
            elif path_value == self.drill_traces_path_var.get():
                self.onedrive_manager._drill_traces_path = path_value if path_value else None
                self.app.config_manager.set("onedrive_drill_traces", path_value)
        ...
        try:
            ...
            if os.path.exists(file_path):
                if not DialogHelper.confirm_dialog(
                    self.root,
                    self.t("File Exists"),
                    self.t("The file already exists. Overwrite it?")
                ):
                    return
            ...
            # ===================================================
            # MODIFIED: Check for correct file location
            # ===================================================
            # The JSONRegisterManager creates the Excel file in the base directory
            # No need to move it if user selected the correct location
            expected_excel = os.path.join(register_dir, register_manager.EXCEL_FILE)
            
            # If user chose a different filename, we need to rename
                ...
                if os.path.exists(expected_excel):
                    try:
                        ...
                        DialogHelper.show_message(
                            self.root,
                            self.t("Error"),
                            self.t(f"Could not rename register file: {str(e)}"),
                            message_type="error"
                        )
                        return
            else:
            ...
            # Update the path in OneDrive manager
            self.onedrive_manager._register_path = file_path
            
            # Save to config
            self.app.config_manager.set("onedrive_register_path", file_path)
            ...
            DialogHelper.show_message(
                ...
                self.t("Success"),
                # ===================================================
                # MODIFIED: Updated message to reflect new structure
                # ===================================================
            ...
            DialogHelper.show_message(
                self.root,
                self.t("Error"),
                self.t(f"An error occurred creating the register: {str(e)}"),
                message_type="error"
            )
    ...
    def _update_onedrive_paths(self):
        ...
        # Show confirmation message using DialogHelper
        DialogHelper.show_message(
            self.root, 
    ...
    def _start_image_review(self):
        ...
        try:
            ...
            if not hasattr(self, 'qaqc_manager') or self.qaqc_manager is None:
                ...
                # ADD: Set main GUI reference for status updates
                self.qaqc_manager.gui = self
            ...
            DialogHelper.show_message(self.root, "Error", f"An error occurred starting the review: {str(e)}", message_type="error")
    ...
    def _start_logging_review(self):
        ...
        try:
            ...
            # First, synchronize registers with OneDrive folders
            ...
            synchronizer = RegisterSynchronizer(
                ...
                self.app.onedrive_manager,
                self.app.config
            )
            
            # Get register path for JSON manager
            register_path = self.app.onedrive_manager.get_register_path()
            if register_path:
                base_path = os.path.dirname(register_path)
                synchronizer.set_json_manager(base_path)
            else:
    ...
    def _check_onedrive_paths(self):
        """Check if OneDrive paths are available and accessible."""
        # Since OneDrivePathManager now checks config first, 
        ...
        if not register_path or not os.path.exists(register_path):
            DialogHelper.show_message(
                self.root,
                self.t("OneDrive Path Error"),
                self.t("Excel register path is not available. Please check OneDrive settings."),
                message_type="error"
            )
            return False
            
        # Check approved folder path
        approved_path = self.onedrive_manager.get_approved_folder_path()
        if not approved_path or not os.path.exists(approved_path):
            DialogHelper.show_message(
                self.root,
                self.t("OneDrive Path Error"),
                self.t("Approved folder path is not available. Please check OneDrive settings."),
                message_type="error"
            )
            return False
    ...
    def check_progress(self, schedule_next=False):
        ...
        # Only schedule the next check if requested
        if schedule_next:
            self.after_id = self.root.after(500, lambda: self.check_progress(schedule_next=True))
