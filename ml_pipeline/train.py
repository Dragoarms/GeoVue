...
Trains compartment classifiers with progress visualization and early stopping.
"""
...
if TORCH_AVAILABLE:
    ...
    def train(
        ...
        """
        Train the compartment classifier.
        ...
        """
        print("=" * 60)
        print("GeoVue ML Pipeline - Compartment Classifier Training")
        print("=" * 60)
        ...
        if config.auto_class_names_from_folders and not config.class_names:
            config.class_names = dataset.get_class_names()
            config.num_classes = len(config.class_names)

        if config.num_classes != len(dataset.CLASS_NAMES):
            raise ValueError(
                f"Config num_classes={config.num_classes} but dataset has "
                f"{len(dataset.CLASS_NAMES)} classes: {', '.join(dataset.CLASS_NAMES)}"
            )

        missing_classes = dataset.missing_classes()
        if missing_classes:
            raise ValueError(
                "Missing required training samples for class(es): "
                f"{', '.join(missing_classes)}. Add images named like "
                "HoleID_CC_001_Empty.png before retraining."
            )

        # Save resolved config after dynamic class discovery.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train compartment classifier")
    parser.add_argument(
        "--shared-folder",
        type=str,
