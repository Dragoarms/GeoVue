...
to classify compartment images as Wet or Dry.
"""
...
class WetDryPredictor:
    """
    Singleton predictor for wet/dry classification.
    ...
    def load_model(self, model_path: Optional[Path] = None) -> bool:
        """
        Load the wet/dry classifier model.
        ...
        try:
            ...
            logger.info(f"Loading wet/dry classifier from: {model_path}")
    ...
    def predict_single(self, image_path: str) -> Tuple[str, float]:
        ...
        Returns:
            Tuple of (label, confidence) where label is "Wet" or "Dry"
        """
    ...
    def predict_review_items(self, items: List) -> Dict[str, Tuple[str, float]]:
        """
        Predict wet/dry for a list of ReviewItems.
        ...
        # Log summary
        wet_count = sum(1 for _, (l, c) in predictions.items() if l == "Wet")
        dry_count = sum(1 for _, (l, c) in predictions.items() if l == "Dry")
        high_conf = sum(1 for _, (l, c) in predictions.items() if c >= self.HIGH_CONFIDENCE_THRESHOLD)

        logger.info(f"ML Predictions: {wet_count} Wet, {dry_count} Dry "
