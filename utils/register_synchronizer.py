...
class RegisterSynchronizer:
    ...
    def __init__(self, file_manager, onedrive_manager, config, progress_callback: Optional[Callable] = None):
        """
        Initialize the register synchronizer.
        
        Args:
            file_manager: FileManager instance
            onedrive_manager: OneDrivePathManager instance
    ...
    def _cache_paths(self):
        ...
        try:
            self._cached_paths['approved_folder'] = self.onedrive_manager.get_approved_folder_path()
            self._cached_paths['processed_originals'] = self.onedrive_manager.get_processed_originals_path()
            self._cached_paths['rejected_folder'] = self.onedrive_manager.get_rejected_folder_path()
            self._cached_paths['register_data'] = self.onedrive_manager.get_register_data_path()
            
            self.logger.info("Cached OneDrive paths for synchronization")
    ...
    def _sync_compartment_images(self) -> Dict:
        ...
        for project_folder in os.listdir(approved_path):
            project_path = os.path.join(approved_path, project_folder)
            ...
            for hole_folder in os.listdir(project_path):
                ...
                for filename in os.listdir(hole_path):
                    ...
                    if match:
                        ...
                        # Calculate depth_from (assuming 1m intervals by default)
                        ...
                        if key not in existing_keys:
                            ...
                            self.logger.info(f"Queued compartment: {hole_id} {depth_from}-{depth_to}m")
    ...
    def _sync_wet_dry_filenames(self) -> Dict:
            ...
            for _, row in wet_dry_entries.iterrows():
                ...
                if found_file and needs_rename:
                    ...
                    old_path = os.path.join(hole_path, found_file)
                    ...
                    if len(name_parts) > 1:
                        ...
                        new_path = os.path.join(hole_path, new_filename)
                        
                        # ===================================================
                        # CHANGED: Use FileManager's safe rename method
                        # ===================================================
